...
import cf4_aggregate_evidence_smc_execution_authorized_v4 as authorized
import pytest
...
def _sealed_temp_git(
    ...
    for index, relative in enumerate(authorized.AUTHORIZATION_V2_PATHS):
        _write(
            repo / relative,
            f"authorization-v4 fixture {index}\n",
    ...
    result_record = {
        ...
        "status": "complete_pass_postcommit_authorization_v4_implementation",
    ...
    _write(release, json.dumps({
        ...
        "authorization_v4_implementation_result_record_sha256": result_sha,
    }, sort_keys=True) + "\n")
    return {
        "repo": repo,
        "remote": remote,
...
def test_authorization_program_exactly_pins_runner_inputs_and_closed_matrix():
    ...
    assert program["runner_implementation_commit"] == (
        "f4e282cb1fe1e80a1184ead23d1fe5892a0c7c5e"
    )
...
def test_public_entry_accepts_no_override_and_refuses_before_base_core(
    ...
    assert set(inspect.signature(authorized.run_authorized_v4).parameters) == {
        "program_path"
    }
    with pytest.raises(PermissionError, match="canonical program"):
        authorized.run_authorized_v4(tmp_path / "wrong.json")
    with pytest.raises(PermissionError, match="grant is absent"):
        authorized.run_authorized_v4(authorized.CANONICAL_PROGRAM)
    assert calls == []
...
def test_temp_git_valid_single_add_grant_invokes_fake_core_once_without_gpfs(
    ...
    monkeypatch.setattr(
        authorized.base_execution,
        "_actual_evaluator_factory",
        lambda program: "evaluator",
    )
    monkeypatch.setattr(
        authorized.base_execution,
        "_execute_into_reserved_directory",
        lambda program, data_directory, **kwargs: calls.append(program) or {"status": "fake-pass"},
    )
    monkeypatch.setattr(
        authorized,
        "CANONICAL_PROGRAM",
    ...
    result = authorized.run_authorized_v4(authorized.CANONICAL_PROGRAM)
...
def test_temp_git_requires_external_non_self_committed_lineage_go(
    ...
    with pytest.raises(PermissionError, match="must not be self-committed"):
        authorized.validate_future_grant(fixture["grant"], {})
def test_actual_authorization_rejects_abbreviated_grant_parent_commit(tmp_path):
    fixture = _sealed_temp_git(tmp_path)
    grant = json.loads(fixture["grant"].read_text())
    grant["grant_parent_commit"] = "346d2f4"
    fixture["grant"].write_text(json.dumps(grant, indent=2) + "\n")
    with pytest.raises(RuntimeError, match="grant parent.*full lowercase Git commit"):
        authorized._validate_git_seal(grant, fixture["grant"])
def test_public_entry_uses_v4_reserved_path_and_frozen_base_callables(monkeypatch):
    captured = {}
    base = {"frozen": True}
    monkeypatch.setattr(authorized, "load_canonical_authorization_program", lambda **kwargs: {})
    monkeypatch.setattr(authorized, "require_execution_authorization", lambda program: {})
    monkeypatch.setattr(authorized.base_execution, "load_canonical_program", lambda **kwargs: base)
    monkeypatch.setattr(authorized.base_execution, "_actual_evaluator_factory", lambda program: "evaluator")
    def execute(program, data_directory, **kwargs):
        captured.update(program=program, data_directory=data_directory, **kwargs)
        return {"status": "complete_pass", "outcome_kind": "test"}
    monkeypatch.setattr(authorized.base_execution, "_execute_into_reserved_directory", execute)
    result = authorized.run_authorized_v4(authorized.CANONICAL_PROGRAM)
    assert result["status"] == "complete_pass"
    assert captured["program"] is base
    assert captured["data_directory"] == authorized.DATA_DIRECTORY
    assert captured["data_directory"] != authorized.ROOT / "gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v1"
    assert captured["validation_runner"] is authorized.base_execution.run_validation
    assert captured["control_runner"] is authorized.base_execution.run_sealed_regression_control
    assert captured["capability_core"] is authorized.base_execution._run_fixed_capability_core
