...
import cf4_lg_highk_terminal_publication_repair_v2 as repair
...
def test_config_is_exact_canonical_and_reserves_absent_grant_and_result() -> None:
    ...
    assert config["resources"]["partition"] == "a10"
    assert "node" not in config["resources"]
    for field in ("grant_path", "future_result_record_path"):
        path = ROOT / config["execution"][field]
        assert not path.exists() and not path.is_symlink()


def test_v1_bundle_and_grant_are_immutable_permanent_nonexecution_record() -> None:
    config = _config()
    record = config["lineage"]["v1_permanent_nonexecution_record"]
    assert record["execution_state"] == "permanently_not_executed"
    assert record["slurm_submitted"] is False
    assert record["canonical_or_staging_mutation_by_v1_repair"] is False
    v1_paths = [
        "config/cf4_lg_highk_terminal_publication_repair_program_v1.json",
        "scripts/check_cf4_lg_highk_terminal_publication_repair_v1.py",
        "scripts/run_cf4_lg_highk_terminal_publication_repair_v1.sbatch",
        "src/cf4_lg_highk_terminal_publication_repair.py",
        "tests/test_cf4_lg_highk_terminal_publication_repair.py",
        "tests/test_cf4_lg_highk_terminal_publication_repair_slurm.py",
        "config/cf4_lg_highk_terminal_publication_repair_grant_v1.json",
    ]
    for relative in v1_paths:
        historical = subprocess.check_output(["git", "show", f"{repair.EXPECTED_PARENT}:{relative}"])
        assert historical == (ROOT / relative).read_bytes()
    assert hashlib.sha256((ROOT / record["grant_path"]).read_bytes()).hexdigest() \
        == record["grant_sha256"]
