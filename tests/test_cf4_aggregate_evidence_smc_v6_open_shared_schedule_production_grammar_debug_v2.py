...
SBATCH = ROOT / "scripts/run_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2.sbatch"
STATUS = ROOT / "scripts/status_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2.sh"
...
SOURCE = ROOT / "src/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_authorized_v2.py"
SOURCE_TEST = ROOT / "tests/test_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_authorized_v2.py"
...
def test_sbatch_header_and_timeout_child_argv_are_exact() -> None:
    ...
    assert "cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_authorized_v2" in text
...
def test_authorization_and_slurm_provenance_precede_child_and_runtime() -> None:
    ...
    assert "s=_read_scheduler_context(); _require_resources(s); _require_runtime_environment()" in text
...
def test_v2_delta_is_confined_to_versioning_scheduler_parser_and_cgroup() -> None:
    source = SOURCE.read_text()
    assert 'details.get("ReqTRES", "")' in source
    assert 'details.get("AllocTRES", "")' in source
    assert 'details.get("TRES", "")' not in source
    assert 'PROC_SELF_CGROUP = Path("/proc/self/cgroup")' in source
    assert 'PROC_SELF_MOUNTINFO = Path("/proc/self/mountinfo")' in source
    assert '"memory.limit_in_bytes"' in source and '"memory.max"' in source
    assert 'target.is_symlink()' in source
    assert '_execute_reserved_canonical_private(' in source
    program = PROGRAM.read_text()
    assert '"maximum_expected_RSS_GiB": 80' in program
    assert '"requested_memory_GiB": 96' in program
    assert '"memory_margin_fraction": 0.2' in program


def test_v1_implementation_files_remain_byte_identical_to_design_pins() -> None:
    import hashlib
    import json

    design = json.loads((ROOT / "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_design_v2.json").read_text())
    rows = design["immutable_v1_chain"]["implementation_files"]
    assert len(rows) == 6
