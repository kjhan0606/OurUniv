...
PROGRAM = ROOT / "config/cf4_phasec_sampler_mechanics_pilot_v3.json"
SOURCE = ROOT / "src/cf4_phasec_sampler_mechanics_pilot_v3.py"
RUNNER = ROOT / "scripts/run_cf4_phasec_sampler_mechanics_v3.sbatch"
AGGREGATOR = ROOT / "scripts/aggregate_cf4_phasec_sampler_mechanics_v3.sbatch"
...
def test_v3_keeps_science_and_truth_contract_but_repairs_mechanics():
    program = load_program()
    ...
    assert sampler["initial_step_size"] == 0.005
...
def test_v3_persists_warmup_diagnostics_and_has_fail_closed_checks():
...
def test_v3_runner_uses_quarantined_slurm_and_memory_headroom():
...
def test_v3_loader_validates_the_frozen_program():
    spec = importlib.util.spec_from_file_location("sampler_v3", SOURCE)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(module)
    program, digest, base = module.load_program(PROGRAM)
    assert program["schema"] == "ouruniv-cf4-phasec-sampler-mechanics-pilot-v3"
    assert digest == sha256(PROGRAM)
    assert base["schema"] == "ouruniv-cf4-datum-bearing-z0-phasec-program-v1"


def _load_source_module():
    spec = importlib.util.spec_from_file_location("sampler_v3_mutation_test", SOURCE)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(module)
    return module
...
def test_v3_validator_rejects_npz_result_consistency_mutation(tmp_path):
    ...
    (root / "manifest.json").write_text(
        json.dumps(module.artifact_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v3-task-manifest-v1"), sort_keys=True)
        + "\n"
    )
    ...
    with pytest.raises(module.SamplerMechanicsV3Error, match="divergence count"):
        module.validate_task(root)
