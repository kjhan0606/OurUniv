...
MEMBERS = ROOT / "scripts/run_cf4_same_truth_ablation_members_v1.sbatch"
AGGREGATE = ROOT / "scripts/run_cf4_same_truth_ablation_aggregate_v1.sbatch"


def test_member_runner_is_exact_cpu_array_with_refuse_overwrite():
    ...
    assert "#SBATCH --mem=2560M" in text
...
def test_aggregate_runner_is_afterok_submission_target_without_aged_dependency_assertion():
    text = AGGREGATE.read_text()
    assert "#SBATCH --cpus-per-task=4" in text
    assert "#SBATCH --mem=4096M" in text
    assert "#SBATCH --time=00:30:00" in text
    assert "MEMBER_ARRAY_JOB_ID" in text
    assert "cf4_same_truth_afterok_${MEMBER_ARRAY_JOB_ID}" in text
    assert "Dependency=afterok:" not in text
    assert '[[ ! -e "$output" ]]' in text
    assert "aggregate" in text and "validate-aggregate" in text


def test_runners_pin_program_source_clean_commit_and_controller_only_execution():
...
def test_program_allows_exactly_one_submission_and_no_followon():
