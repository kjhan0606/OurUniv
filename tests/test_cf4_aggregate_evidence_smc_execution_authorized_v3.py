...
import cf4_aggregate_evidence_smc_execution_authorized_v3 as authorized
import pytest
...
def _sealed_temp_git(
    ...
    for index, relative in enumerate(authorized.AUTHORIZATION_V2_PATHS):
        _write(
            repo / relative,
            f"authorization-v3 fixture {index}\n",
    ...
    result_record = {
        ...
        "status": "complete_pass_postcommit_authorization_v3_implementation",
    ...
    _write(release, json.dumps({
        ...
        "authorization_v3_implementation_result_record_sha256": result_sha,
    }, sort_keys=True) + "\n")
    return {
        "repo": repo,
        "remote": remote,
...
def test_public_entry_accepts_no_override_and_refuses_before_base_core(
    ...
    assert set(inspect.signature(authorized.run_authorized_v3).parameters) == {
        "program_path"
    }
    with pytest.raises(PermissionError, match="canonical program"):
        authorized.run_authorized_v3(tmp_path / "wrong.json")
    with pytest.raises(PermissionError, match="grant is absent"):
        authorized.run_authorized_v3(authorized.CANONICAL_PROGRAM)
    assert calls == []
...
def test_temp_git_valid_single_add_grant_invokes_fake_core_once_without_gpfs(
    ...
    result = authorized.run_authorized_v3(authorized.CANONICAL_PROGRAM)
