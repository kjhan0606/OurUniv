...
SBATCH = ROOT / "scripts/run_cf4_lg_highk_terminal_publication_repair_v1.sbatch"
CONFIG = ROOT / "config/cf4_lg_highk_terminal_publication_repair_program_v1.json"
RUNNER = ROOT / "src/cf4_lg_highk_terminal_publication_repair.py"
...
def test_slurm_is_exact_debug_cpu_only_grammar() -> None:
    ...
    assert not any("--gres" in line or "--gpus" in line or "--array" in line for line in directives)
    assert "nvidia-smi" not in text
    config = json.loads(CONFIG.read_text())
    assert config["resources"] == {
        "partition": "debug", "node": "grammar-debug", "nodes": 1, "tasks": 1,
...
def test_slurm_uses_only_fixed_circle_python_and_three_fail_closed_phases() -> None:
    ...
    assert 'os.environ["SLURM_JOB_PARTITION"] == "debug"' in text
    assert 'os.environ["SLURMD_NODENAME"] == "grammar-debug"' in text
