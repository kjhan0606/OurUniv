...
from hong2021_v45_development_gate import classify
...
from hong2021_v45_sample import ENSEMBLE_SCHEMA
from hong2021_v45_train import PARAMETERS, PROGRAM_SHA256
...
def test_program_hash_firewall_and_evaluator_schema() -> None:
    path = REPO / "config/hong2021_v45_identifiable_mixture_copula_program.json"
...
def test_network_shape_parameter_count_and_identifiable_initial_output() -> None:
    ...
    variance = torch.sum(
        weights
        * (torch.square(locations - mean[:, None]) + np.pi**2 * torch.square(scales) / 3),
        dim=1,
    )
...
def test_identifiable_initialization_breaks_v44_gradient_symmetry() -> None:
    ...
    old_loss = -logistic_mixture_log_probability(
    ...
    new_loss = -logistic_mixture_log_probability(
...
def test_classification_branches() -> None:
    assert classify(True, True, True, True, False)[0] == "identifiable_query_local_mixture_copula_sufficient"
    assert (
