...
import cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_authorized_v1 as authorized
...
def _scheduler(tmp_path: Path, job_id: str = "731") -> dict:
    ...
    return {
        ...
        "minimum_memory_node": "96G", "tres": "cpu=12,mem=96G,node=1,billing=12",
...
def test_scheduler_context_is_exact_and_allocation_mutations_fail(
    ...
    details = {
        ...
        "TRES": "cpu=12,mem=96G,node=1,billing=12",
    ...
    assert authorized._read_scheduler_context()["memory_MiB"] == 98304
    for key, bad in (
    ...
    for key, bad in (
        ...
        ("MinMemoryNode", "95G"), ("TRES", "cpu=12,mem=95G,node=1,billing=12"),
    ):
        old = details[key]
        details[key] = bad
        with pytest.raises(PermissionError):
