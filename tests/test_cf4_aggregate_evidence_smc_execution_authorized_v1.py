...
import cf4_aggregate_evidence_smc_execution_authorized_v1 as authorized
import pytest
...
def _sealed_temp_git(
    ...
    for index, relative in enumerate(authorized.AUTHORIZATION_V1_PATHS):
        _write(
            repo / relative,
            f"authorization-v1 fixture {index}\n",
            executable=relative.endswith(".sh"),
        )
    _git(repo, "add", *authorized.AUTHORIZATION_V1_PATHS)
    _git(repo, "commit", "-m", "authorization implementation")
    implementation_commit = _git(repo, "rev-parse", "HEAD")
    rows = []
    for relative in authorized.AUTHORIZATION_V1_PATHS:
        path = repo / relative
    ...
    if intermediate_auth_change:
        changed = repo / authorized.AUTHORIZATION_V1_PATHS[2]
        changed.write_text(changed.read_text() + "intermediate mutation\n")
        _git(repo, "add", authorized.AUTHORIZATION_V1_PATHS[2])
    ...
    result_record = {
        ...
        "status": "complete_pass_postcommit_authorization_v1_implementation",
    ...
    _write(release, json.dumps({
        ...
        "authorization_v1_implementation_result_record_sha256": result_sha,
    }, sort_keys=True) + "\n")
    return {
        "repo": repo,
        "remote": remote,
...
def test_authorization_program_exactly_pins_runner_inputs_and_closed_matrix():
    ...
    assert program["runner_implementation_commit"] == (
        "3db0e219a2569ed300e3647eca94a7078cae073c"
    )
...
def test_public_entry_accepts_no_override_and_refuses_before_base_core(
    ...
    assert set(inspect.signature(authorized.run_authorized_v1).parameters) == {
        "program_path"
    }
    with pytest.raises(PermissionError, match="canonical program"):
        authorized.run_authorized_v1(tmp_path / "wrong.json")
    with pytest.raises(PermissionError, match="grant is absent"):
        authorized.run_authorized_v1(authorized.CANONICAL_PROGRAM)
    assert calls == []
...
def test_temp_git_valid_single_add_grant_invokes_fake_core_once_without_gpfs(
    ...
    monkeypatch.setattr(
        authorized,
        "CANONICAL_PROGRAM",
        fixture["repo"] / authorized.AUTHORIZATION_V1_PATHS[1],
    )
    result = authorized.run_authorized_v1(authorized.CANONICAL_PROGRAM)
