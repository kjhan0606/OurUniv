...
RUNNER = ROOT / "scripts/run_cf4_aggregate_evidence_smc_authorized_v2_lageunha.sh"
LAUNCHER = ROOT / "scripts/launch_cf4_aggregate_evidence_smc_authorized_v2_lageunha.sh"
...
def test_authorized_runner_is_fixed_lageunha_fail_closed_and_resource_bounded():
    ...
    assert "375438fa6dc911059da57e46be95183ee45f1837" in text
    assert "cf4_aggregate_evidence_smc_execution_authorization_program_v2.json" in text
    assert "cf4_aggregate_evidence_smc_execution_grant_v2.json" in text
...
def test_launcher_refuses_locally_before_ssh_tmux_or_namespace_checks():
    ...
    assert "readonly session=cf4-aggregate-evidence-smc-authorized-v2" in text
    assert "run_cf4_aggregate_evidence_smc_authorized_v2_lageunha.sh" in text
    assert not re.search(r"\b(?:pgrep|ps)\b", text)
    assert "sleep " not in text
...
def test_existing_status_checker_is_reused_unchanged_with_fake_namespaces(tmp_path):
    ...
    assert not (
        ROOT / "scripts/status_cf4_aggregate_evidence_smc_authorized_v2.sh"
    ).exists()
