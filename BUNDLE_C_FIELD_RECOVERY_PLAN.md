...
Fable5 review completed normally in137035ms: CONDITIONAL GO. The three
...
Reuse the existing metrics' units, parent references and binning. Use eight
8-cell-side blocks centered at the combinations of n/4 and3n/4 in each axis
(n=32,64,128 across the scored scales), at the same indices in X and Y.
Freeze these indices and training-only scales in one run config BEFORE the
first update. Each feature group is normalized by
...
Freeze a single structural coefficient from the training-only gradient check
below, NOT objective magnitudes. Record both objective values and gradient
norms; no heldout optimization or later adjustment of this weight.
...
training-only gradient/time check. Each four-pair half covers the three
native-parent transitions plus one complete worst-case full-context rollout
through .1875. Thus full-rollout log-probability backpropagation and peak memory
are exercised BEFORE branch training, not first encountered at update8.
Evaluate terms with sequential node backpropagation where possible; detached
traces do not require retaining all node computation graphs.

Form each half's mean structural gradient with weights7/24 on each one-step
gradient and1/8 on the full-rollout gradient, matching the training score
mixture. Let g_ES be the mean of the two half estimates. Let g_N be the mean
native NLL gradient on one fixed training record per original scale, using
the first training origin/node0 and the original mean-per-parent convention.
Use unclipped gradients. Freeze lambda=||g_N||/||g_ES|| once; zero or nonfinite
norms/ratio stop the trial, not trigger new sample selection. The original
gradient clip10 remains identical in both branches after loss combination.
This operational balance of noisy pilot means does not measure the true
expected gradient ratio or establish effective signal-to-noise.

Record finite/nonzero structural gradients, norms and split-half direction
agreement, not an invented scientific SNR pass. A positive split-half dot
product can occur under pure noise; passing means ONLY no gross pathology
detected, not estimator viability. Terminal field evaluations decide usefulness.
...
all results at start/end; one midpoint native NLL only. For every completed
branch, INCLUDING both-fail outcomes, tabulate train/retained true-parent
versus rollout high-band means at .75/.375/.1875. In early-stop or incomplete
outcomes, include existing starting values and explicitly unavailable end
values; do not conceal missing results or run extra fits to fill the table.
This separates observed one-step and accumulated deficits, but does not uniquely
identify exposure bias, capacity or optimization as their cause.
Show individual density
...
inference at the current capability. Fable's suggested coarse-z0-posterior +
prior fine IC phases + forward zoom is recorded as a possible CHANGE of scope,
not an automatic fallback or a solution to missing LG observational constraints.
It would require an explicit MW/M31/M33 conditioning mechanism and separate
user approval; the route remains present-field inference first. Do not
relabel another near-identical flow run or more M33 diagnostics as a new central
milestone. A failed bounded repair closes this chosen trial, not every possible
method or the scientific feasibility of the project.
