...
CANONICAL_PROGRAM = (
    ROOT / "config/cf4_aggregate_evidence_smc_execution_authorization_program_v2.json"
)
PROGRAM_SHA256 = "b028cf09045a46061f899dc1a8b0212a1ebe586e0b5d27f3f159a47f48d3a244"
CANONICAL_DESIGN = (
    ROOT / "config/cf4_aggregate_evidence_smc_execution_authorization_design_v2.json"
)
DESIGN_SHA256 = "80751efb175b140a767d7ac70cd327b79a74ed0b8b33552e1971a4c740a78985"
GRANT_RELATIVE_PATH = "config/cf4_aggregate_evidence_smc_execution_grant_v2.json"
...
AUTHORIZATION_RESULT_RELATIVE_PATH = (
    ...
    "result_record_v2.json"
)
EXTERNAL_RELEASE = Path(
    "/gpfs/kjhan/CF4/recon/linear_cr/"
    "aggregate_evidence_smc_execution_authorization_v2_release.json"
)
AUTHORIZATION_DESIGN_BASE_COMMIT = "d3213fa8fa2effe82dc6874911d21132dc088b4b"
RUNNER_IMPLEMENTATION_COMMIT = "375438fa6dc911059da57e46be95183ee45f1837"
RUNNER_RESULT_PATH = (
...
DATA_DIRECTORY = Path(
    "/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v2"
)
STATE_DIRECTORY = Path(
    "/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v2_run"
)
...
GRANT_KEYS = {
    ...
    "authorization_v2_implementation_commit",
    "authorization_v2_implementation_result_record",
    "authorization_v2_files",
    "data_directory",
    "state_directory",
    "authorization",
...
def _validate_design() -> None:
    ...
    if design.get("schema") != (
        "ouruniv-cf4-aggregate-evidence-smc-execution-authorization-design-v2"
...
def validate_authorization_program(
    ...
    if program.get("schema") != (
        "ouruniv-cf4-aggregate-evidence-smc-execution-authorization-program-v2"
    ...
    if grant != {
        ...
        "grant_parent_commit_source": (
            "The commit containing the immutable authorization-v2 "
            "implementation result record."
        ),
        "authorization_v2_implementation_result_record_path": (
            AUTHORIZATION_RESULT_RELATIVE_PATH
        ),
        "prospective_values_resolved_from_grant_parent_not_self_pinned": True,
        "exact_commit_chain": (
            "grant_parent^ == authorization_v2_implementation_commit and "
...
def _validate_result_record(grant: dict[str, Any], *, parent: str) -> str:
    binding = grant.get("authorization_v2_implementation_result_record")
    if not isinstance(binding, dict) or set(binding) != {"path", "sha256"} \
            or binding.get("path") != AUTHORIZATION_RESULT_RELATIVE_PATH:
        raise RuntimeError("authorization-v2 implementation result binding changed")
    expected_sha = _require_sha256(
        binding.get("sha256"), "authorization-v2 implementation result record"
    )
    ...
    if not record_path.is_file() or record_path.read_bytes() != tracked_record \
        ...
        raise RuntimeError("authorization-v2 implementation result record changed")
    record = json.loads(tracked_record)
    implementation_commit = _require_commit(
        grant.get("authorization_v2_implementation_commit"),
        "authorization-v2 implementation commit",
    )
    ...
    if implementation_parent != implementation_commit:
        raise RuntimeError(
            "grant parent is not the direct child of the authorization-v2 "
    ...
    if record.get("schema") != (
        ...
        "complete_pass_postcommit_authorization_v2_implementation"
    ) or record.get("commit_lineage", {}).get("git_commit") != (
        implementation_commit
    ):
        raise RuntimeError("authorization-v2 implementation result lineage changed")
    rows = record.get("committed_authorization_files")
    if grant.get("authorization_v2_files") != rows \
            or not isinstance(rows, list) or len(rows) != 7 \
            or [row.get("path") for row in rows] != list(AUTHORIZATION_V2_PATHS):
        raise RuntimeError("authorization-v2 seven-file binding changed")
    for row in rows:
        if not isinstance(row, dict) or set(row) != {
            "path", "sha256", "git_blob_oid", "git_mode"
        }:
            raise RuntimeError("authorization-v2 file record schema changed")
        relative = row["path"]
        ...
        if len(blob) != 40 \
            ...
            raise RuntimeError(f"authorization-v2 file changed: {relative}")
    return expected_sha
...
def _validate_external_lineage_release(
    ...
    expected_keys = {
        ...
        "grant_sha256", "authorization_v2_implementation_result_record_sha256",
    }
    if set(release) != expected_keys \
            ...
            or release.get(
                "authorization_v2_implementation_result_record_sha256"
            ) != result_sha:
...
def validate_future_grant(
    ...
    if set(grant) != GRANT_KEYS or grant.get("schema") != (
            "ouruniv-cf4-aggregate-evidence-smc-execution-grant-v2"
...
def run_authorized_v2(program_path: Path) -> dict[str, Any]:
    ...
    if path != CANONICAL_PROGRAM.resolve():
        raise PermissionError("authorized v2 accepts only the canonical program path")
...
def main() -> None:
    ...
    result = run_authorized_v2(arguments.program)
    print(
