#!/usr/bin/env python
"""Sample the frozen V48 Gaussian mixture with unchanged donor ranks."""
...
ENSEMBLE_SCHEMA = "hong2021-v48-identifiable-query-local-Gaussian-mixture-copula-ensemble-v1"
ARMS = (
    "identifiable_query_local_gaussian_mixture_copula",
...
def load_fit(
    ...
    if sha256_file(checkpoint_path) != checkpoint_sha:
        raise ValueError("V48 checkpoint hash differs")
    report = _verified_json(report_path, report_sha, "V48 report")
    preflight = _verified_json(preflight_path, preflight_sha, "V48 preflight")
    ...
    if (
        ...
        or checkpoint.get("likelihood_family") != "Gaussian"
        ...
        raise ValueError("V48 checkpoint metadata differs")
    if (
        ...
        or report.get("likelihood_family") != "Gaussian"
        ...
        raise ValueError("V48 report or preflight binding differs")
    if sha256_file(cache_path) != cache_sha:
        raise ValueError("V48 cache hash differs")
...
def sample_all(
    ...
    if not clean:
        raise RuntimeError("V48 sampling requires a clean worktree")
    if output_root.exists():
        raise FileExistsError("V48 refuses existing output root")
    ...
    if not torch.cuda.is_available() or "ada" not in torch.cuda.get_device_name(0).lower():
        raise RuntimeError("V48 sampling requires the Lageunha Ada GPU")
    device = torch.device("cuda")
    model = LocalMixtureUNet().to(device)
    if parameter_count(model) != PARAMETERS:
        raise RuntimeError("V48 sampling architecture differs")
        ...
        for domain in DOMAIN_ORDER:
            ...
            try:
                ...
                for object_index, query_index in enumerate(indices):
                    ...
                    parameter_by_arm = {
                        "identifiable_query_local_gaussian_mixture_copula": actual_parameter,
                    ...
                    for arm in ARMS:
                        ...
                        cdf_error = torch.amax(
                            torch.abs(
                                gaussian_mixture_cdf(parameters, standardized)
                        ...
                        if not np.isfinite(sample).all() or float(dc.max()) > 1.0e-7:
                            raise RuntimeError("V48 sampled field or DC differs")
                    ...
                    print(f"[v48-sample] {domain} {object_index + 1}/16", flush=True)
                for arm in ARMS:
