...
CANONICAL_PROGRAM = (
    ROOT / "config/cf4_aggregate_evidence_smc_execution_authorization_program_v1.json"
)
PROGRAM_SHA256 = "99539c20d873125aaece88239fb5615a1929db5f98e2fd84eb13f13da64bd841"
CANONICAL_DESIGN = (
    ROOT / "config/cf4_aggregate_evidence_smc_execution_authorization_design_v1.json"
)
DESIGN_SHA256 = "a37a981dd12feaad7a85608a68e0a3b0f57ee4c1d15b385112286b5cb6608bef"
GRANT_RELATIVE_PATH = "config/cf4_aggregate_evidence_smc_execution_grant_v1.json"
...
AUTHORIZATION_RESULT_RELATIVE_PATH = (
    ...
    "result_record_v1.json"
)
EXTERNAL_RELEASE = Path(
    "/gpfs/kjhan/CF4/recon/linear_cr/"
    "aggregate_evidence_smc_execution_authorization_v1_release.json"
)
AUTHORIZATION_DESIGN_BASE_COMMIT = "d3213fa8fa2effe82dc6874911d21132dc088b4b"
RUNNER_IMPLEMENTATION_COMMIT = "3db0e219a2569ed300e3647eca94a7078cae073c"
RUNNER_RESULT_PATH = (
...
AUTHORIZATION_V1_PATHS = (
    "config/cf4_aggregate_evidence_smc_execution_authorization_design_v1.json",
    "config/cf4_aggregate_evidence_smc_execution_authorization_program_v1.json",
    "src/cf4_aggregate_evidence_smc_execution_authorized_v1.py",
    "tests/test_cf4_aggregate_evidence_smc_execution_authorized_v1.py",
    "scripts/run_cf4_aggregate_evidence_smc_authorized_v1_lageunha.sh",
    "scripts/launch_cf4_aggregate_evidence_smc_authorized_v1_lageunha.sh",
    "tests/test_cf4_aggregate_evidence_smc_authorized_v1_runner.py",
)
...
GRANT_KEYS = {
    ...
    "authorization_v1_implementation_commit",
    "authorization_v1_implementation_result_record",
    "authorization_v1_files",
    "data_directory",
    "state_directory",
    "authorization",
...
EXPECTED_RUNNER_FILES = (
    ...
    (
        ...
        "17d85585dfbba18ca9c5684141ba75ba9e1ccb93c0c89dc5231f5b0ba25fe4a6",
        "bc52f3b93849b62d1d3faf7905987557eeb700a5",
        "100755",
    ),
    (
    ...
    (
        ...
        "679db0908ed135a3c969a1f50267f0d1fbccf25af9f01d8a47bbcc892b041786",
        "4fd7196e148e4d51e855073a958daec758cc41d8",
        "100644",
    ),
)
...
def _validate_design() -> None:
    ...
    if design.get("schema") != (
        "ouruniv-cf4-aggregate-evidence-smc-execution-authorization-design-v1"
...
def validate_authorization_program(
    ...
    if program.get("schema") != (
        "ouruniv-cf4-aggregate-evidence-smc-execution-authorization-program-v1"
    ...
    if grant != {
        ...
        "schema": "ouruniv-cf4-aggregate-evidence-smc-execution-grant-v1",
        ...
        "grant_parent_commit_source": (
            "The commit containing the immutable authorization-v1 "
            "implementation result record."
        ),
        "authorization_v1_implementation_result_record_path": (
            AUTHORIZATION_RESULT_RELATIVE_PATH
        ),
        "prospective_values_resolved_from_grant_parent_not_self_pinned": True,
        "exact_commit_chain": (
            "grant_parent^ == authorization_v1_implementation_commit and "
...
def _validate_result_record(grant: dict[str, Any], *, parent: str) -> str:
    binding = grant.get("authorization_v1_implementation_result_record")
    if not isinstance(binding, dict) or set(binding) != {"path", "sha256"} \
            or binding.get("path") != AUTHORIZATION_RESULT_RELATIVE_PATH:
        raise RuntimeError("authorization-v1 implementation result binding changed")
    expected_sha = _require_sha256(
        binding.get("sha256"), "authorization-v1 implementation result record"
    )
    ...
    if not record_path.is_file() or record_path.read_bytes() != tracked_record \
        ...
        raise RuntimeError("authorization-v1 implementation result record changed")
    record = json.loads(tracked_record)
    implementation_commit = _require_commit(
        grant.get("authorization_v1_implementation_commit"),
        "authorization-v1 implementation commit",
    )
    ...
    if implementation_parent != implementation_commit:
        raise RuntimeError(
            "grant parent is not the direct child of the authorization-v1 "
    ...
    if record.get("schema") != (
        ...
        "complete_pass_postcommit_authorization_v1_implementation"
    ) or record.get("commit_lineage", {}).get("git_commit") != (
        implementation_commit
    ):
        raise RuntimeError("authorization-v1 implementation result lineage changed")
    rows = record.get("committed_authorization_files")
    if grant.get("authorization_v1_files") != rows \
            or not isinstance(rows, list) or len(rows) != 7 \
            or [row.get("path") for row in rows] != list(AUTHORIZATION_V1_PATHS):
        raise RuntimeError("authorization-v1 seven-file binding changed")
    for row in rows:
        if not isinstance(row, dict) or set(row) != {
            "path", "sha256", "git_blob_oid", "git_mode"
        }:
            raise RuntimeError("authorization-v1 file record schema changed")
        relative = row["path"]
        ...
        if len(blob) != 40 \
            ...
            raise RuntimeError(f"authorization-v1 file changed: {relative}")
    return expected_sha
...
def _validate_external_lineage_release(
    ...
    expected_keys = {
        ...
        "grant_sha256", "authorization_v1_implementation_result_record_sha256",
    }
    if set(release) != expected_keys \
            ...
            or release.get(
                "authorization_v1_implementation_result_record_sha256"
            ) != result_sha:
...
def validate_future_grant(
    ...
    if set(grant) != GRANT_KEYS or grant.get("schema") != (
        "ouruniv-cf4-aggregate-evidence-smc-execution-grant-v1"
...
def run_authorized_v1(program_path: Path) -> dict[str, Any]:
    ...
    if path != CANONICAL_PROGRAM.resolve():
        raise PermissionError("authorized v1 accepts only the canonical program path")
    ...
    return base_execution._execute_authorized_program(base_program)
...
def main() -> None:
    ...
    result = run_authorized_v1(arguments.program)
    print(
