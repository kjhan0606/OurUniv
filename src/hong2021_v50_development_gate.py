#!/usr/bin/env python
"""Integrity-bound three-domain development gate for V50."""
...
from hong2021_v50_sample import ARMS, ENSEMBLE_SCHEMA, METHOD
...
SCHEMA = "hong2021-v50-bounded-query-local-mixture-copula-three-domain-decision-v1"
...
def _validate(
    ...
    with h5py.File(path, "r") as current, h5py.File(parent, "r") as old:
        exact = {
            ...
            "method": METHOD,
            "arm": arm,
            "v50_program_sha256": PROGRAM_SHA256,
        ...
        for key, expected in exact.items():
            if _value(current.attrs.get(key)) != expected:
                raise ValueError(f"V50 {domain} {arm} metadata differs: {key}")
        if not (
            LOWER_SUPPORT
            ...
            raise ValueError("V50 pre-DC sample support differs")
        reused = (
            "source_index",
            "donor_source",
            "donor_index",
        ...
        if tuple(current["sample"].shape) != (16, 16, 1, 64, 64, 64) or any(
            ...
            raise ValueError("V50 ensemble shape or frozen selection differs")
        ...
        if maximum_dc > 1.0e-7 or inverse_error > 2.0e-6:
            raise ValueError("V50 residual DC or inverse CDF differs")
        ...
        if (
            ...
            raise ValueError("V50 artifact hash differs")
        ...
        if (
            ...
            or not checked.get("bounded_physical_moment_proof")
        ...
        if subprocess.run(
            ...
            raise ValueError("V50 sampling commit is not an ancestor")
...
def classify(
    ...
    if primary:
        return (
            ...
            "seal_v50_and_await_explicit_approval_before_independent_gate",
        )
    if q3 and q4:
        return (
            "bounded_local_marginal_is_calibrated_but_empirical_rank_copula_limits_morphology",
            "audit_only_conditional_rank_copula_dependence_without_changing_the_bounded_marginal_likelihood",
        )
    if high_k_rms:
        return (
            "bounded_marginal_support_preserves_the_field_body_but_is_not_sufficient_for_extreme_calibration",
            "audit_train_only_support_boundary_occupancy_and_bounded_PIT_without_threshold_or_support_changes",
        )
    if rolled_improves:
        return (
...
def evaluate(root: Path, program_path: Path, repo: Path, commit: str) -> dict[str, Any]:
    ...
    for arm in ARMS:
        ...
        for domain in DOMAIN_ORDER:
            ...
            if Path(metrics["path"]).resolve() != ensemble.resolve():
                raise ValueError("V50 metrics point elsewhere")
            domains[domain] = {
    ...
    for domain in DOMAIN_ORDER:
        ...
        for arm in ARMS[1:]:
            ...
            if (
                ...
                raise ValueError("V50 arms changed donor ranks or object amplitude")
    ...
    result: dict[str, Any] = {
        "schema": SCHEMA,
        "experiment": "v50_bounded_query_local_mixture_copula",
...
def main() -> None:
    ...
    if not clean:
        raise RuntimeError("V50 gate requires a clean committed worktree")
    result = evaluate(
        args.root.resolve(), args.program.resolve(), args.repo.resolve(), commit
    )
    if args.out.exists():
        raise FileExistsError("V50 refuses existing decision")
