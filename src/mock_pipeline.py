...
def make_forward(
    ...
    return_dens=True,
    cosmology=None,
    return_particle_arrays=False,
    return_particle_positions=False,
    ...
    def forward(s):
        ...
        if return_particle_arrays or return_particle_positions:
            # Materialize only the two arrays required by large PM screens.
            # Returning the full Particles pytree also exports pmid and disp;
            # at N=576 those extra output buffers can exceed a 32-GiB GPU.
            pos = ptcl.pos(dtype=dtype)
            if return_particle_positions:
                return pos if particle_indices is None else pos[particle_indices]
            vel = ptcl.vel
            if particle_indices is not None:
                pos = pos[particle_indices]
                vel = vel[particle_indices]
            return pos, vel
