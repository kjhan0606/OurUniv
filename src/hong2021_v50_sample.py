#!/usr/bin/env python
"""Sample the frozen V50 bounded likelihood with unchanged donor ranks."""
...
ENSEMBLE_SCHEMA = "hong2021-v50-bounded-query-local-mixture-copula-ensemble-v1"
METHOD = "train_only_bounded_query_local_logit_Gaussian_mixture_empirical_rank_copula"
ARMS = (
...
def load_fit(
    ...
    if sha256_file(checkpoint_path) != checkpoint_sha:
        raise ValueError("V50 checkpoint hash differs")
    ...
    if (
        ...
        raise ValueError("V50 checkpoint metadata differs")
    if (
        ...
        raise ValueError("V50 report or preflight binding differs")
    if sha256_file(cache_path) != cache_sha:
        raise ValueError("V50 cache hash differs")
...
def sample_all(
    ...
    if not clean:
        raise RuntimeError("V50 sampling requires a clean worktree")
    if output_root.exists():
        raise FileExistsError("V50 refuses existing output root")
    ...
    if not torch.cuda.is_available() or "ada" not in torch.cuda.get_device_name(0).lower():
        raise RuntimeError("V50 sampling requires the Lageunha Ada GPU")
    device = torch.device("cuda")
    model = LocalMixtureUNet().to(device)
    if parameter_count(model) != PARAMETERS:
        raise RuntimeError("V50 sampling architecture differs")
        ...
        for domain in DOMAIN_ORDER:
            ...
            try:
                ...
                for object_index, query_index in enumerate(indices):
                    ...
                    parameter_by_arm = {
                        "bounded_query_local_mixture_copula": actual_parameter,
                    ...
                    for arm in ARMS:
                        ...
                        if not bool(
                            ...
                            raise RuntimeError("V50 sample left frozen support")
                        ...
                        if not np.isfinite(sample).all() or float(dc.max()) > 1.0e-7:
                            raise RuntimeError("V50 sampled field or DC differs")
                    ...
                    print(f"[v50-sample] {domain} {object_index + 1}/16", flush=True)
                for arm in ARMS:
