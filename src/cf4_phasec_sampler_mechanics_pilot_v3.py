#!/usr/bin/env python3
"""Mock-only sampler-mechanics V3 with conservative initialization and warm-up diagnostics."""
...
SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-pilot-v3"
TASK_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v3-task-v1"
AGGREGATE_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v3-aggregate-v1"
TASK_FILES = {"diagnostics.npz", "result.json", "manifest.json", "COMPLETE"}
AGGREGATE_FILES = {"aggregate.json", "manifest.json", "COMPLETE"}


class SamplerMechanicsV3Error(ValueError):
...
def load_program(path: str | Path) -> tuple[dict[str, object], str, dict[str, object]]:
    ...
    try:
        ...
        raise SamplerMechanicsV3Error("cannot parse v3 sampler program") from exc
    if controller.get("schema") != SCHEMA:
        raise SamplerMechanicsV3Error("v3 sampler schema mismatch")
    auth = controller.get("authorization", {})
    for key in (
        "replacement_Phase_C_mock_sampler_mechanics_v3",
        ...
        if auth.get(key) is not True:
            raise SamplerMechanicsV3Error(f"missing v3 authorization: {key}")
    for key in (
        ...
        if auth.get(key) is not False:
            raise SamplerMechanicsV3Error(f"forbidden v3 scope enabled: {key}")
    if controller.get("assignments") != expected_assignments():
        raise SamplerMechanicsV3Error("v3 sampler assignments changed")
    expected_lineage = {
        "base_program",
        "replacement_program",
    ...
    if set(lineage) != expected_lineage:
        raise SamplerMechanicsV3Error("v3 lineage set changed")
    for name, binding in lineage.items():
        ...
        if expected_hash.startswith("TO_BE_FILLED"):
            raise SamplerMechanicsV3Error("v3 implementation is not frozen")
        if not source_path.is_file() or sha256_file(source_path) != expected_hash:
            raise SamplerMechanicsV3Error(f"v3 lineage mismatch: {name}")

    mechanics = controller.get("mechanics", {})
    if mechanics.get("latent_dimension") != 32792:
        raise SamplerMechanicsV3Error("v3 latent dimension changed")
    expected_map = {
    ...
    if mechanics.get("MAP") != expected_map:
        raise SamplerMechanicsV3Error("v3 MAP mechanics changed")
    expected_sampler = {
    ...
    if mechanics.get("sampler") != expected_sampler:
        raise SamplerMechanicsV3Error("v3 sampler mechanics changed")
    if mechanics.get("field_probe_indices") != [0, 1, 31, 32, 1024, 4096, 16384, 32767]:
        raise SamplerMechanicsV3Error("v3 field probes changed")
    expected_gates = {
    ...
    if controller.get("gates") != expected_gates:
        raise SamplerMechanicsV3Error("v3 sampler gates changed")
    if controller.get("truth_integrator") != {
        ...
        raise SamplerMechanicsV3Error("v3 truth integrator changed")
    ...
    if (
        ...
        raise SamplerMechanicsV3Error("v3 hardware quarantine changed")

    base_path = Path(controller["lineage"]["base_program"]["path"])
    base = json.loads(base_path.read_text())
    if base.get("schema") != v1.phasec_v5.SCHEMA:
        raise SamplerMechanicsV3Error("base mock schema mismatch")
    for binding in base.get("input_bindings", {}).values():
        ...
        if not input_path.is_file() or sha256_file(input_path) != binding["sha256"]:
            raise SamplerMechanicsV3Error(f"base mock input mismatch: {input_path}")
    ...
    if base.get("mock_assignments") != expected_base_assignments:
        raise SamplerMechanicsV3Error("base mock assignments changed")
    for key in (
        "actual_observational_field_inference",
        "actual_2Mpp_count_read",
        "actual_CF4_velocity_datum_used",
        ...
        if base.get("authorization", {}).get(key) is not False:
            raise SamplerMechanicsV3Error(f"base mock forbidden scope enabled: {key}")
    return controller, hashlib.sha256(payload).hexdigest(), copy.deepcopy(base)


def task_name(assignment: Mapping[str, object]) -> str:
    return (
        f"mechanics_v3_{int(assignment['task_index']):02d}_mock_{int(assignment['mock_index']):02d}"
...
def validate_manifest(directory: Path, schema: str) -> None:
    ...
    if manifest.get("schema") != schema:
        raise SamplerMechanicsV3Error("v3 manifest schema mismatch")
...
def run_identity_hmc(
    ...
    if not math.isfinite(map_value) or not np.all(np.isfinite(map_gradient)):
        raise SamplerMechanicsV3Error("v3 MAP value or gradient is nonfinite")
    map_gradient_rms = float(np.linalg.norm(map_gradient) / math.sqrt(map_gradient.size))
    if not np.all(np.isfinite(map_position)):
        raise SamplerMechanicsV3Error("v3 MAP position is nonfinite")
    ...
    for chain in range(chain_count):
        ...
        if not math.isfinite(start_value) or not np.all(np.isfinite(start_gradient)):
            raise SamplerMechanicsV3Error(f"v3 chain {chain} initial value or gradient is nonfinite")
        ...
        if not np.all(np.isfinite(positions_np)):
            raise SamplerMechanicsV3Error(f"v3 chain {chain} contains nonfinite positions")
    ...
    if warmup_steps % block_size != 0:
        raise SamplerMechanicsV3Error("v3 warmup steps must be divisible by diagnostic block size")
...
def run_task(
    ...
    if task_index not in (0, 1) or len(implementation_commit) != 40:
        raise SamplerMechanicsV3Error("invalid v3 task index or commit")
    assignment = controller["assignments"][task_index]
    output = Path(output_root) / task_name(assignment)
    staging = output.parent / f".{output.name}.staging"
    if output.exists() or staging.exists():
        raise SamplerMechanicsV3Error("v3 output or staging already exists")
    ...
    if jax.default_backend() != "gpu" or len(jax.devices()) != 1:
        raise SamplerMechanicsV3Error("v3 task requires one Slurm GPU")
    ...
    warmup_divergence_fraction = float(
        ...
        / (draws.shape[0] * int(controller["mechanics"]["sampler"]["warmup_steps"]))
    )
    ...
    result = {
        "schema": TASK_SCHEMA,
        "status": "PASS_SAMPLER_MECHANICS_V3_TASK" if pilot_pass else "NO_GO_SAMPLER_MECHANICS_V3_TASK",
    ...
    (staging / "manifest.json").write_text(
        json.dumps(artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v3-task-manifest-v1"), indent=2, sort_keys=True)
        + "\n"
    )
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v3-task-complete-v1",
...
def validate_task(directory: str | Path) -> dict[str, object]:
    ...
    if not root.is_dir() or {path.name for path in root.iterdir()} != TASK_FILES:
        raise SamplerMechanicsV3Error("v3 task artifact set mismatch")
    result = json.loads((root / "result.json").read_text())
    if result.get("schema") != TASK_SCHEMA or not isinstance(result.get("pilot_pass"), bool):
        raise SamplerMechanicsV3Error("v3 task result schema mismatch")
    complete = json.loads((root / "COMPLETE").read_text())
    if complete.get("result_sha256") != sha256_file(root / "result.json"):
        raise SamplerMechanicsV3Error("v3 task result hash mismatch")
    if complete.get("manifest_sha256") != sha256_file(root / "manifest.json"):
        raise SamplerMechanicsV3Error("v3 task manifest hash mismatch")
    if complete.get("pilot_pass") != result["pilot_pass"]:
        raise SamplerMechanicsV3Error("v3 task decision mismatch")
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v3-task-manifest-v1")
    ...
    with np.load(root / "diagnostics.npz", allow_pickle=False) as archive:
        ...
        if not required.issubset(archive.files):
            raise SamplerMechanicsV3Error("v3 warmup traces are missing")
        for name in (
            "sampler_warmup_energy",
            "sampler_warmup_is_divergent",
            "sampler_warmup_acceptance_rate",
            ...
            if np.asarray(archive[name]).shape != expected_warmup_shape:
                raise SamplerMechanicsV3Error("v3 warmup trace shape mismatch")
        if np.asarray(archive["sampler_warmup_divergence_count"]).shape != (4,):
            raise SamplerMechanicsV3Error("v3 warmup count shape mismatch")
        for name in (
            "sampler_warmup_energy_finite_by_block",
            "sampler_warmup_divergence_fraction_by_block",
        ):
            if np.asarray(archive[name]).shape != expected_warmup_block_shape:
                raise SamplerMechanicsV3Error("v3 warmup block trace shape mismatch")
        for name in (
            "sampler_warmup_initialization_value",
            "sampler_warmup_initialization_gradient_RMS",
        ):
            if np.asarray(archive[name]).shape != (4,):
                raise SamplerMechanicsV3Error("v3 warmup initialization trace shape mismatch")
        ...
        if sampling_is_divergent.shape != expected_sampling_shape or sampling_energy.shape != expected_sampling_shape:
            raise SamplerMechanicsV3Error("v3 sampling trace shape mismatch")
        ...
        if not isinstance(sampler, dict) or not isinstance(checks, dict):
            raise SamplerMechanicsV3Error("v3 task sampler/check records are missing")
        if not np.array_equal(
            np.asarray(sampler.get("warmup_divergence_count"), dtype=int),
            warmup_divergence_count,
        ):
            raise SamplerMechanicsV3Error("v3 warmup divergence count does not match NPZ")
        if float(sampler.get("warmup_divergence_fraction")) != warmup_divergence_fraction:
            raise SamplerMechanicsV3Error("v3 warmup divergence fraction does not match NPZ")
        if not np.array_equal(
            np.asarray(sampler.get("warmup_divergence_fraction_by_block"), dtype=np.float64),
            warmup_divergence_fraction_by_block,
        ):
            raise SamplerMechanicsV3Error("v3 warmup block divergence trace does not match NPZ")
        if not np.array_equal(
            np.asarray(sampler.get("warmup_energy_finite_by_block"), dtype=bool),
            warmup_energy_finite_by_block,
        ):
            raise SamplerMechanicsV3Error("v3 warmup block energy trace does not match NPZ")
        if not np.array_equal(
            ...
            raise SamplerMechanicsV3Error("v3 initialization trace does not match NPZ")
        if checks.get("all_warmup_energies_finite") is not warmup_energy_finite:
            raise SamplerMechanicsV3Error("v3 warmup energy finiteness gate does not match NPZ")
        if checks.get("warmup_divergence_fraction") is not (
            warmup_divergence_fraction <= 0.01
        ):
            raise SamplerMechanicsV3Error("v3 warmup divergence gate does not match NPZ")
        if float(sampler.get("sampling_divergence_fraction")) != sampling_divergence_fraction:
            raise SamplerMechanicsV3Error("v3 sampling divergence fraction does not match NPZ")
        if checks.get("divergence_fraction") is not (
            sampling_divergence_fraction <= 0.01
        ):
            raise SamplerMechanicsV3Error("v3 sampling divergence gate does not match NPZ")
        if checks.get("all_sampling_energies_finite") is not sampling_energy_finite:
            raise SamplerMechanicsV3Error("v3 sampling energy finiteness gate does not match NPZ")
        if sampler.get("warmup_energy_trace_stored") is not True or checks.get(
            "warmup_energy_trace_stored"
        ) is not True:
            raise SamplerMechanicsV3Error("v3 warmup trace storage gate is not true")
        if checks.get("warmup_energy_trace_stored") is not True:
            raise SamplerMechanicsV3Error("v3 warmup trace storage check is inconsistent")
        if bool(checks) and bool(result.get("pilot_pass")) != bool(all(checks.values())):
            raise SamplerMechanicsV3Error("v3 pilot pass does not match task checks")
    if result["checks"].get("warmup_energy_trace_stored") is not True:
        raise SamplerMechanicsV3Error("v3 warmup energy trace storage gate failed")
    return result
...
def aggregate(
    ...
    if len(implementation_commit) != 40:
        raise SamplerMechanicsV3Error("v3 aggregate commit must be full hash")
    implementation_sha = sha256_file(__file__)
    output = Path(aggregate_output)
    staging = output.parent / f".{output.name}.staging"
    if output.exists() or staging.exists():
        raise SamplerMechanicsV3Error("v3 aggregate output already exists")
    outcomes = []
    for assignment in controller["assignments"]:
        ...
        try:
            ...
            if result.get("assignment") != assignment:
                raise SamplerMechanicsV3Error("v3 task assignment mismatch")
            if result.get("program", {}).get("sha256") != program_sha:
                raise SamplerMechanicsV3Error("v3 task program hash mismatch")
            implementation = result.get("implementation", {})
            if implementation.get("path") != str(Path(__file__).resolve()):
                raise SamplerMechanicsV3Error("v3 task implementation path mismatch")
            if implementation.get("sha256") != implementation_sha:
                raise SamplerMechanicsV3Error("v3 task implementation hash mismatch")
            if implementation.get("commit") != implementation_commit:
                raise SamplerMechanicsV3Error("v3 task implementation commit mismatch")
            outcomes.append(
                {
                    "assignment": assignment,
    ...
    result = {
        ...
        "status": "PASS_BOTH_SAMPLER_MECHANICS_V3_TASKS" if aggregate_pass else "NO_GO_SAMPLER_MECHANICS_V3_STOP",
        "program": {"path": str(Path(program_path).resolve()), "sha256": program_sha},
    ...
    (staging / "manifest.json").write_text(
        json.dumps(artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v3-aggregate-manifest-v1"), indent=2, sort_keys=True)
        + "\n"
    )
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v3-aggregate-complete-v1",
...
def validate_aggregate(directory: str | Path) -> dict[str, object]:
    ...
    if not root.is_dir() or {path.name for path in root.iterdir()} != AGGREGATE_FILES:
        raise SamplerMechanicsV3Error("v3 aggregate artifact set mismatch")
    result = json.loads((root / "aggregate.json").read_text())
    if result.get("schema") != AGGREGATE_SCHEMA or result.get("task_count") != 2:
        raise SamplerMechanicsV3Error("v3 aggregate schema mismatch")
    complete = json.loads((root / "COMPLETE").read_text())
    if complete.get("aggregate_sha256") != sha256_file(root / "aggregate.json"):
        raise SamplerMechanicsV3Error("v3 aggregate hash mismatch")
    if complete.get("manifest_sha256") != sha256_file(root / "manifest.json"):
        raise SamplerMechanicsV3Error("v3 aggregate manifest hash mismatch")
    if complete.get("both_pilot_tasks_pass") != result.get("both_pilot_tasks_pass"):
        raise SamplerMechanicsV3Error("v3 aggregate decision mismatch")
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v3-aggregate-manifest-v1")
    return result
