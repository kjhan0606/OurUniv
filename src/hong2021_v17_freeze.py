#!/usr/bin/env python
"""Create and verify the committed V17/Astrid one-shot artifact seal."""
...
from hong2021_v14_freeze import (
    _file_row, _load_json, _run_git, _tracked_protocol_rows,
    astrid_files, sha256,
)
from hong2021_v15_development_gate import canonical_digest


SCHEMA = "hong2021-v17-astrid-one-shot-artifact-seal-v1"
...
DEFAULT_DESTINATION = Path("config/hong2021_v17_astrid_one_shot_seal.json")
EXACT_ONE_SHOT_COMMAND = (
    "scripts/run_hong2021_v17_astrid_one_shot_lageunha.sh "
    "config/hong2021_v17_astrid_one_shot_seal.json"
)
...
def _selected_artifacts(
    ...
    sequence_path = tng / "evaluation/tng100_simba_swift_v17_sequence/status.json"
    sequence = _load_json(sequence_path)
    if sequence.get("state") != "complete_e5_passed_astrid_still_unopened":
        raise RuntimeError("V17 has no completed passing three-domain development gate")
    decision_path = Path(sequence["detail"]).resolve()
    decision = _load_json(decision_path)
    if decision.get("experiment") != "e5_band_balanced_denoising":
        raise RuntimeError("V17 sequence refers to the wrong experiment")
    if decision.get("development_pass") is not True:
        raise RuntimeError("V17 development decision is not passing")
    if canonical_digest(decision) != decision.get("decision_digest_sha256"):
        raise RuntimeError("V17 development decision digest is invalid")
    if decision.get("Astrid_used") is not False:
        raise RuntimeError("V17 decision does not prove Astrid remained unused")
    ...
    provenance = {
        "v17_sequence": sequence_path,
        "development_decision": decision_path,
        "v17_registry": repo / "config/hong2021_v17_development_program.json",
...
def create_seal(
    *, repo: Path, tng: Path, astrid_root: Path, destination: Path
    ...
    if astrid_files(astrid_root):
        raise RuntimeError("Astrid must remain unopened before the V17 seal commit")
    ...
    if artifact_rows["edm"]["sha256"] != selection["selected"]["checkpoint_sha256"]:
        raise RuntimeError("selected EDM changed after the V17 development gate")
    ...
    if edm.get("schema") != V17_E5_SCHEMA:
        raise ValueError("selected EDM is not the V17-E5 equal-band model")
    if edm.get("decoder_upsampling") != "nearest" or edm.get("decoder_align_corners") is not None:
        raise ValueError("selected EDM lacks the frozen nearest decoder invariant")
    loss = edm.get("denoising_loss", {})
    ...
    record = {
        ...
        "astrid_preopen": {
            "root": str(astrid_root.resolve()), "files_found": 0,
            "content_read": False,
        },
        "astrid_raw_units": {
        ...
        "one_shot": {
            ...
            "ensemble": 16, "sampling_steps": 40,
            "sigma_min": 0.002, "sigma_max": 40.0, "rho": 7.0,
            "seed": 28777,
            "field_checks": "unchanged eight checks from hong2021_v6_gate.field_gate",
            "grid_hop_only_after_field_pass": True,
            "hop_members": 16, "hop_objects": 27, "hop_omega_m": 0.3,
            "hop_bootstrap": 50000, "hop_bootstrap_seed": 2024,
        ...
        "commit_requirement": "This record must be the only file in a commit directly atop the frozen V17 code before any Astrid file is downloaded or opened.",
    }
...
def verify_seal(
    ...
    if record.get("schema") != SCHEMA:
        raise ValueError("not a V17 Astrid one-shot seal")
    ...
    if require_committed:
        ...
        if len(commits) != 1:
            raise RuntimeError("the V17 seal must be added once and never modified")
        ...
        if committed != path.read_bytes():
            raise RuntimeError("the working V17 seal differs from its committed blob")
    ...
    if record["one_shot"]["command"] != EXACT_ONE_SHOT_COMMAND:
        raise RuntimeError("V17 one-shot command differs from the verifier")
    if require_unopened and astrid_files(Path(record["astrid_preopen"]["root"])):
        raise RuntimeError("Astrid was already opened before V17 seal verification")
    return record
