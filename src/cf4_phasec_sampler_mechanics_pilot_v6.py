#!/usr/bin/env python3
"""Mock-only sampler-mechanics V6 with conservative initialization and warm-up diagnostics."""
...
SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-pilot-v6"
TASK_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v6-task-v1"
AGGREGATE_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v6-aggregate-v1"
...
class SamplerMechanicsV6Error(ValueError):
    """The frozen L=32 sampler-mechanics contract was violated."""


SamplerMechanicsV5Error = SamplerMechanicsV6Error
...
def _as_int(value: object, *, field: str) -> int:
    if not isinstance(value, int):
        raise SamplerMechanicsV6Error(f"v6 assignment {field} must be integer")
    return value
...
def _validate_assignments(
    ...
    *,
    ...
    if len(assignments) != expected_mock_count:
        raise SamplerMechanicsV6Error("v6 assignment count does not match expected mock count")
    normalized = []
    for assignment in assignments:
        if set(assignment.keys()) != {"task_index", "mock_index", "seed", "arm"}:
            raise SamplerMechanicsV6Error("v6 assignment schema mismatch")
        task_index = _as_int(assignment.get("task_index"), field="task_index")
        mock_index = _as_int(assignment.get("mock_index"), field="mock_index")
        seed = _as_int(assignment.get("seed"), field="seed")
        arm = assignment.get("arm")
        if not isinstance(arm, str) or len(arm) != 1 or arm not in {"A", "B", "C", "D"}:
            raise SamplerMechanicsV6Error("v6 assignment arm must be one of A/B/C/D")
        normalized.append(
            {
    ...
    if sorted({entry["task_index"] for entry in normalized}) != expected_indices:
        raise SamplerMechanicsV6Error("v6 task_index must be contiguous from 0")
    for entry in normalized:
        expected_seed = 2026083000 + entry["mock_index"]
        if entry["seed"] != expected_seed:
            raise SamplerMechanicsV6Error("v6 assignment seed must be 2026083000 + mock_index")
        if entry["arm"] != "ABCD"[entry["mock_index"] // 2]:
            raise SamplerMechanicsV6Error("v6 assignment arm must match mock-index arm mapping")
    if len({entry["mock_index"] for entry in normalized}) != len(normalized):
        raise SamplerMechanicsV6Error("v6 mock indices must be unique")
    if not all(0 <= entry["mock_index"] <= 7 for entry in normalized):
        raise SamplerMechanicsV6Error("v6 mock index out of allowed range")
    normalized.sort(key=lambda entry: entry["task_index"])
    return normalized
...
def load_program(path: str | Path) -> tuple[dict[str, object], str, dict[str, object]]:
    ...
    try:
        ...
        raise SamplerMechanicsV5Error("cannot parse v6 sampler program") from exc
    if controller.get("schema") != SCHEMA:
        raise SamplerMechanicsV5Error("v6 sampler schema mismatch")
    ...
    for key in (
        ...
        if auth.get(key) is not True:
            raise SamplerMechanicsV5Error(f"missing v6 authorization: {key}")
    for key in (
        ...
        if auth.get(key) is not False:
            raise SamplerMechanicsV5Error(f"forbidden v6 scope enabled: {key}")
    assignment_count = len(controller.get("assignments", []))
    if assignment_count <= 0:
        raise SamplerMechanicsV5Error("v6 must include at least one assignment")
    controller["assignments"] = _validate_assignments(
        list(controller.get("assignments", [])),
        expected_mock_count=int(assignment_count),
    )
    expected_lineage = {
        "base_program",
        "replacement_program",
...
def task_name(assignment: Mapping[str, object]) -> str:
    return (
        f"mechanics_v6_{int(assignment['task_index']):02d}_mock_{int(assignment['mock_index']):02d}"
...
def run_task(
    ...
    task_count = len(controller["assignments"])
    if not (0 <= task_index < task_count) or len(implementation_commit) != 40:
        raise SamplerMechanicsV5Error("invalid v6 task index or commit")
    assignment = next(
        assignment_item
        for assignment_item in controller["assignments"]
        if int(assignment_item["task_index"]) == task_index
    )
    ...
    result = {
        "schema": TASK_SCHEMA,
        "status": "PASS_SAMPLER_MECHANICS_V6_TASK" if pilot_pass else "NO_GO_SAMPLER_MECHANICS_V6_TASK",
    ...
    (staging / "manifest.json").write_text(
        json.dumps(artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v6-task-manifest-v1"), indent=2, sort_keys=True)
        + "\n"
    )
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v6-task-complete-v1",
...
def validate_task(directory: str | Path) -> dict[str, object]:
    ...
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v6-task-manifest-v1")
...
def aggregate(
    ...
    if len(implementation_commit) != 40:
        raise SamplerMechanicsV5Error("v6 aggregate commit must be full hash")
    implementation_sha = sha256_file(__file__)
    output = Path(aggregate_output)
    staging = output.parent / f".{output.name}.staging"
    if output.exists() or staging.exists():
        raise SamplerMechanicsV5Error("v6 aggregate output already exists")
    outcomes = []
    ...
    expected_task_count = len(controller["assignments"])
    aggregate_pass = bool(len(outcomes) == expected_task_count and all(row["pilot_pass"] for row in outcomes))
    result = {
        "schema": AGGREGATE_SCHEMA,
        "status": "PASS_ALL_SAMPLER_MECHANICS_V6_TASKS" if aggregate_pass else "NO_GO_SAMPLER_MECHANICS_V6_STOP",
        "program": {"path": str(Path(program_path).resolve()), "sha256": program_sha},
    ...
    (staging / "manifest.json").write_text(
        json.dumps(
            artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v6-aggregate-manifest-v1"),
            indent=2,
            sort_keys=True,
        )
        + "\n"
    ...
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v6-aggregate-complete-v1",
...
def validate_aggregate(directory: str | Path) -> dict[str, object]:
    ...
    if not root.is_dir() or {path.name for path in root.iterdir()} != AGGREGATE_FILES:
        raise SamplerMechanicsV5Error("v6 aggregate artifact set mismatch")
    result = json.loads((root / "aggregate.json").read_text())
    if result.get("schema") != AGGREGATE_SCHEMA:
        raise SamplerMechanicsV5Error("v6 aggregate schema mismatch")
    if not isinstance(result.get("task_count"), int) or result["task_count"] <= 0:
        raise SamplerMechanicsV5Error("v6 aggregate task_count is invalid")
    complete = json.loads((root / "COMPLETE").read_text())
    if complete.get("aggregate_sha256") != sha256_file(root / "aggregate.json"):
        raise SamplerMechanicsV5Error("v6 aggregate hash mismatch")
    if complete.get("manifest_sha256") != sha256_file(root / "manifest.json"):
        raise SamplerMechanicsV5Error("v6 aggregate manifest hash mismatch")
    if complete.get("both_pilot_tasks_pass") != result.get("both_pilot_tasks_pass"):
        raise SamplerMechanicsV5Error("v6 aggregate decision mismatch")
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v6-aggregate-manifest-v1")
    return result
