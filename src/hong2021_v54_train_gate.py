#!/usr/bin/env python
"""Train-only high-backbone mechanism gate for the frozen V54 score."""
...
SCHEMA = "hong2021-v54-train-only-high-backbone-mechanism-decision-v1"
...
def _load_fit(
    ...
    if sha256_file(checkpoint_path) != checkpoint_sha:
        raise ValueError("V54 mechanism checkpoint hash differs")
    checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    report = json.loads(report_path.read_text())
    if sha256_file(report_path) != report_sha or sha256_file(preflight_path) != preflight_sha:
        raise ValueError("V54 mechanism report or preflight hash differs")
    ...
    if (
        ...
        or subprocess.run(["git", "merge-base", "--is-ancestor", source_commit, commit], cwd=repo, capture_output=True).returncode
    ):
        raise ValueError("V54 mechanism fit binding differs")
    load_thresholds(threshold_path, threshold_sha, source_commit)
    model = LocalMixtureUNet()
    if parameter_count(model) != PARAMETERS:
        raise RuntimeError("V54 mechanism architecture differs")
...
def _domain(
    ...
    if objects != EXPECTED_OBJECTS[domain]:
        raise RuntimeError("V54 mechanism object count differs")
    ...
    nodes64_t, weights64_t = torch.from_numpy(nodes64).to(device), torch.from_numpy(weights64).to(device)
    nodes32_t, weights32_t = torch.from_numpy(nodes32).to(device), torch.from_numpy(weights32).to(device)
    ...
    try:
        ...
        for object_index in range(objects):
            condition, _, backbone = condition_cube(data, cache, prepared, domain, "train", object_index)
            ...
            flat = parameter.reshape(1, 15, -1).index_select(2, index_tensor).reshape(1, 15, 1, 1, -1)
            logits, locations, scales = mixture_parameters(flat)
            mixture_weights = torch.softmax(logits, dim=1)[0, :, 0, 0]
            locations, scales = locations[0, :, 0, 0], scales[0, :, 0, 0]
            base = torch.from_numpy(backbone.reshape(-1)[indices].astype(np.float64) + target_mean).to(device)
            primary = _quadrature_object(mixture_weights, locations, scales, base, target_std, nodes64_t, weights64_t, domain_maximum)
            control = _quadrature_object(mixture_weights, locations, scales, base, target_std, nodes32_t, weights32_t, domain_maximum)
            p = primary["delta_squared"].sum(axis=0)
            c = control["delta_squared"].sum(axis=0)
            primary_parts.append(p)
            primary_sum += float(p.sum(dtype=np.float64))
            control_sum += float(c.sum(dtype=np.float64))
            if (object_index + 1) % 16 == 0 or object_index + 1 == objects:
                print(f"[v54-train-gate] {domain} {object_index + 1}/{objects}", flush=True)
    finally:
        data.close()
        cache.close()
    ...
    if not np.allclose(boundaries, sealed["V50_sealed"]["boundaries"], rtol=0.0, atol=1.0e-7):
        raise ValueError("V54 mechanism boundaries differ")
    rows: dict[str, Any] = {}
    for label, mask in zip(STRATUM_LABELS, _masks(variable, boundaries), strict=True):
        ...
        v50 = float(sealed["V50_sealed"]["strata"][label]["quadrature_over_truth_mean_delta_squared"])
        v52 = float(sealed["V52"]["strata"][label]["quadrature_over_truth_mean_delta_squared"])
        rows[label] = {
            "count": int(mask.sum()),
            "truth_mean_delta_squared": truth_mean,
            "V54_quadrature_mean_delta_squared": predicted_mean,
            "V54_over_truth_mean_delta_squared": ratio,
            "V50_over_truth_mean_delta_squared": v50,
            "V52_over_truth_mean_delta_squared": v52,
            "V54_strictly_improves_V50": abs(math.log(ratio)) < abs(math.log(v50)),
            "V54_strictly_improves_V52": abs(math.log(ratio)) < abs(math.log(v52)),
        }
    convergence = _relative_difference(primary_sum, control_sum)
    if convergence > QUADRATURE_MAXIMUM:
        raise RuntimeError("V54 mechanism quadrature convergence differs")
    top_ratio = float(rows["q99_9_and_above"]["V54_over_truth_mean_delta_squared"])
    return {
...
def evaluate(
    ...
    if not clean or socket.gethostname().split(".")[0].lower() != "lageunha":
        raise RuntimeError("V54 mechanism gate requires clean Lageunha")
    if not torch.cuda.is_available() or "ada" not in torch.cuda.get_device_name(0).lower():
        raise RuntimeError("V54 mechanism gate requires Ada")
    model, checkpoint = _load_fit(program, checkpoint_path, checkpoint_sha, report_path, report_sha, threshold_path, threshold_sha, preflight_path, preflight_sha, cache_sha, repo.resolve(), commit)
    model = model.to("cuda").eval()
    prepared = load_cache(cache_path, cache_sha, str(checkpoint["code_commit"]))
    support = json.loads(Path(program["frozen_inputs"]["support_selection"]).read_text())
    v53 = json.loads(Path(program["frozen_inputs"]["v53_audit"]).read_text())
    domains: dict[str, Any] = {}
    try:
        for domain_index, domain in enumerate(DOMAIN_ORDER):
            domains[domain] = _domain(model, torch.device("cuda"), v35, prepared, support, v53, domain, domain_index)
    finally:
        prepared.close()
    passed = all(row["top_backbone_pass"] for row in domains.values())
    result: dict[str, Any] = {
        "schema": SCHEMA,
        ...
        "classification": (
            ...
            if passed
            else "proper_exceedance_tail_score_does_not_calibrate_train_high_backbone_physical_moments"
        ),
        "next": (
            "proceed_to_locked_V54_development_sampling"
            if passed
            else "stop_before_development_sampling_and_audit_which_fixed_threshold_or_beyond_threshold_amplitudes_dominate_the_remaining_train_moment"
        ),
...
def main() -> None:
    ...
    if args.out.exists():
        raise FileExistsError("V54 refuses existing train mechanism gate")
    result = evaluate(args.program, args.repo, args.cache, args.cache_sha256, args.thresholds, args.thresholds_sha256, args.preflight, args.preflight_sha256, args.checkpoint, args.checkpoint_sha256, args.report, args.report_sha256)
