#!/usr/bin/env python3
"""Mock-only sampler-mechanics V4 with conservative initialization and warm-up diagnostics."""
...
SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-pilot-v4"
TASK_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v4-task-v1"
AGGREGATE_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v4-aggregate-v1"
...
class SamplerMechanicsV4Error(ValueError):
...
def load_program(path: str | Path) -> tuple[dict[str, object], str, dict[str, object]]:
    ...
    try:
        ...
        raise SamplerMechanicsV4Error("cannot parse v4 sampler program") from exc
    if controller.get("schema") != SCHEMA:
        raise SamplerMechanicsV4Error("v4 sampler schema mismatch")
    auth = controller.get("authorization", {})
    for key in (
        "replacement_Phase_C_mock_sampler_mechanics_v4",
        ...
        if auth.get(key) is not True:
            raise SamplerMechanicsV4Error(f"missing v4 authorization: {key}")
    for key in (
        ...
        if auth.get(key) is not False:
            raise SamplerMechanicsV4Error(f"forbidden v4 scope enabled: {key}")
    if controller.get("assignments") != expected_assignments():
        raise SamplerMechanicsV4Error("v4 sampler assignments changed")
    expected_lineage = {
        "base_program",
        "replacement_program",
    ...
    if set(lineage) != expected_lineage:
        raise SamplerMechanicsV4Error("v4 lineage set changed")
    for name, binding in lineage.items():
        ...
        if expected_hash.startswith("TO_BE_FILLED"):
            raise SamplerMechanicsV4Error("v4 implementation is not frozen")
        if not source_path.is_file() or sha256_file(source_path) != expected_hash:
            raise SamplerMechanicsV4Error(f"v4 lineage mismatch: {name}")

    mechanics = controller.get("mechanics", {})
    if mechanics.get("latent_dimension") != 32792:
        raise SamplerMechanicsV4Error("v4 latent dimension changed")
    expected_map = {
    ...
    if mechanics.get("MAP") != expected_map:
        raise SamplerMechanicsV4Error("v4 MAP mechanics changed")
    expected_sampler = {
    ...
    if mechanics.get("sampler") != expected_sampler:
        raise SamplerMechanicsV4Error("v4 sampler mechanics changed")
    if mechanics.get("field_probe_indices") != [0, 1, 31, 32, 1024, 4096, 16384, 32767]:
        raise SamplerMechanicsV4Error("v4 field probes changed")
    expected_gates = {
    ...
    if controller.get("gates") != expected_gates:
        raise SamplerMechanicsV4Error("v4 sampler gates changed")
    if controller.get("truth_integrator") != {
        ...
        raise SamplerMechanicsV4Error("v4 truth integrator changed")
    ...
    if (
        ...
        raise SamplerMechanicsV4Error("v4 hardware quarantine changed")

    base_path = Path(controller["lineage"]["base_program"]["path"])
    base = json.loads(base_path.read_text())
    if base.get("schema") != v1.phasec_v5.SCHEMA:
        raise SamplerMechanicsV4Error("base mock schema mismatch")
    for binding in base.get("input_bindings", {}).values():
        ...
        if not input_path.is_file() or sha256_file(input_path) != binding["sha256"]:
            raise SamplerMechanicsV4Error(f"base mock input mismatch: {input_path}")
    ...
    if base.get("mock_assignments") != expected_base_assignments:
        raise SamplerMechanicsV4Error("base mock assignments changed")
    for key in (
        "actual_observational_field_inference",
        "actual_2Mpp_count_read",
        "actual_CF4_velocity_datum_used",
        ...
        if base.get("authorization", {}).get(key) is not False:
            raise SamplerMechanicsV4Error(f"base mock forbidden scope enabled: {key}")
    return controller, hashlib.sha256(payload).hexdigest(), copy.deepcopy(base)


def task_name(assignment: Mapping[str, object]) -> str:
    return (
        f"mechanics_v4_{int(assignment['task_index']):02d}_mock_{int(assignment['mock_index']):02d}"
...
def validate_manifest(directory: Path, schema: str) -> None:
    ...
    if manifest.get("schema") != schema:
        raise SamplerMechanicsV4Error("v4 manifest schema mismatch")
...
def run_identity_hmc(
    ...
    if not math.isfinite(map_value) or not np.all(np.isfinite(map_gradient)):
        raise SamplerMechanicsV4Error("v4 MAP value or gradient is nonfinite")
    map_gradient_rms = float(np.linalg.norm(map_gradient) / math.sqrt(map_gradient.size))
    if not np.all(np.isfinite(map_position)):
        raise SamplerMechanicsV4Error("v4 MAP position is nonfinite")
    ...
    def warmup_chain(key, start):
        ...
        def one_step(carry, transition_key):
            ...
            adapted_step_size = jnp.exp(current_adaptation.log_step_size)
            # Make the first retained diagnostic an exact record of the frozen
            # initial value.  Subsequent transitions use dual-averaging output.
            step_size = jnp.where(
                step_index == 0,
            ...
            record = (step_size, info.acceptance_rate, info.is_divergent, info.energy)
    ...
    for chain in range(chain_count):
        ...
        if not math.isfinite(start_value) or not np.all(np.isfinite(start_gradient)):
            raise SamplerMechanicsV4Error(f"v4 chain {chain} initial value or gradient is nonfinite")
        ...
        warmup_step_size, warmup_accept, warmup_divergent, warmup_energy = warmup_records
        ...
        if not np.all(np.isfinite(positions_np)):
            raise SamplerMechanicsV4Error(f"v4 chain {chain} contains nonfinite positions")
        draws.append(positions_np)
        step_sizes.append(float(step_size))
        warmup_accept_np = np.asarray(warmup_accept, dtype=np.float64)
        warmup_step_size_np = np.asarray(warmup_step_size, dtype=np.float64)
        ...
        warmup_step_size_traces.append(warmup_step_size_np)
    ...
    warmup_step_size_traces_np = np.asarray(warmup_step_size_traces, dtype=np.float64)
    warmup_divergence_flags_np = np.asarray(warmup_divergence_flags, dtype=bool)
    block_size = 128
    if warmup_steps % block_size != 0:
        raise SamplerMechanicsV4Error("v4 warmup steps must be divisible by diagnostic block size")
...
def run_task(
    ...
    if task_index not in (0, 1) or len(implementation_commit) != 40:
        raise SamplerMechanicsV4Error("invalid v4 task index or commit")
    assignment = controller["assignments"][task_index]
    output = Path(output_root) / task_name(assignment)
    staging = output.parent / f".{output.name}.staging"
    if output.exists() or staging.exists():
        raise SamplerMechanicsV4Error("v4 output or staging already exists")
    ...
    if jax.default_backend() != "gpu" or len(jax.devices()) != 1:
        raise SamplerMechanicsV4Error("v4 task requires one Slurm GPU")
    ...
    warmup_trace_stored = bool(
        ...
        == expected_warmup_shape
        and np.asarray(sampler_arrays["warmup_step_size"]).shape
        == expected_warmup_shape
    )
    warmup_step_size_np = np.asarray(sampler_arrays["warmup_step_size"], dtype=np.float64)
    warmup_step_size_finite_positive = bool(
        np.all(np.isfinite(warmup_step_size_np)) and np.all(warmup_step_size_np > 0.0)
    )
    warmup_step_size_initial_match = bool(
        warmup_step_size_finite_positive
        and warmup_step_size_np.shape == expected_warmup_shape
        and np.array_equal(
            warmup_step_size_np[:, 0],
            np.full(draws.shape[0], float(sampler_config["initial_step_size"]), dtype=np.float64),
        )
    )
    sampling_step_size_np = np.asarray(sampler_arrays["sampling_step_size"], dtype=np.float64)
    sampling_step_size_finite_positive = bool(
        sampling_step_size_np.shape == (draws.shape[0],)
        and np.all(np.isfinite(sampling_step_size_np))
        and np.all(sampling_step_size_np > 0.0)
    )
    checks = {
        ...
        "warmup_step_size_trace_stored": bool(
            warmup_step_size_initial_match and warmup_trace_stored
        ),
        "sampling_step_size_trace_stored": sampling_step_size_finite_positive,
    ...
    result = {
        "schema": TASK_SCHEMA,
        "status": "PASS_SAMPLER_MECHANICS_V4_TASK" if pilot_pass else "NO_GO_SAMPLER_MECHANICS_V4_TASK",
        ...
        "sampler": {
            ...
            "sampling_step_size": np.asarray(sampler_arrays["sampling_step_size"]).tolist(),
            "sampling_step_size_trace_stored": sampling_step_size_finite_positive,
            ...
            "warmup_step_size_first_used": np.asarray(
                sampler_arrays["warmup_step_size"]
            )[:, 0].tolist(),
            "warmup_step_size_last_used": np.asarray(
                sampler_arrays["warmup_step_size"]
            )[:, -1].tolist(),
            "warmup_step_size_trace_stored": bool(
                warmup_step_size_initial_match and warmup_trace_stored
    ...
    np.savez_compressed(
        ...
        **derived_arrays,
        ...
        sampler_warmup_step_size=np.asarray(sampler_arrays["warmup_step_size"]),
        sampler_sampling_step_size=np.asarray(sampler_arrays["sampling_step_size"]),
    ...
    (staging / "manifest.json").write_text(
        json.dumps(artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v4-task-manifest-v1"), indent=2, sort_keys=True)
        + "\n"
    )
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v4-task-complete-v1",
...
def validate_task(directory: str | Path) -> dict[str, object]:
    ...
    if not root.is_dir() or {path.name for path in root.iterdir()} != TASK_FILES:
        raise SamplerMechanicsV4Error("v4 task artifact set mismatch")
    result = json.loads((root / "result.json").read_text())
    if result.get("schema") != TASK_SCHEMA or not isinstance(result.get("pilot_pass"), bool):
        raise SamplerMechanicsV4Error("v4 task result schema mismatch")
    complete = json.loads((root / "COMPLETE").read_text())
    if complete.get("result_sha256") != sha256_file(root / "result.json"):
        raise SamplerMechanicsV4Error("v4 task result hash mismatch")
    if complete.get("manifest_sha256") != sha256_file(root / "manifest.json"):
        raise SamplerMechanicsV4Error("v4 task manifest hash mismatch")
    if complete.get("pilot_pass") != result["pilot_pass"]:
        raise SamplerMechanicsV4Error("v4 task decision mismatch")
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v4-task-manifest-v1")
    ...
    with np.load(root / "diagnostics.npz", allow_pickle=False) as archive:
        required = {
            ...
            "sampler_warmup_step_size",
            "sampler_sampling_step_size",
        ...
        if not required.issubset(archive.files):
            raise SamplerMechanicsV4Error("v4 warmup traces are missing")
        for name in (
            "sampler_warmup_energy",
            "sampler_warmup_is_divergent",
            "sampler_warmup_acceptance_rate",
            ...
            if np.asarray(archive[name]).shape != expected_warmup_shape:
                raise SamplerMechanicsV4Error("v4 warmup trace shape mismatch")
        if np.asarray(archive["sampler_warmup_divergence_count"]).shape != (4,):
            raise SamplerMechanicsV4Error("v4 warmup count shape mismatch")
        for name in (
            "sampler_warmup_energy_finite_by_block",
            "sampler_warmup_divergence_fraction_by_block",
        ):
            if np.asarray(archive[name]).shape != expected_warmup_block_shape:
                raise SamplerMechanicsV4Error("v4 warmup block trace shape mismatch")
        for name in (
            "sampler_warmup_initialization_value",
            "sampler_warmup_initialization_gradient_RMS",
        ):
            if np.asarray(archive[name]).shape != (4,):
                raise SamplerMechanicsV4Error("v4 warmup initialization trace shape mismatch")
        ...
        warmup_step_size = np.asarray(archive["sampler_warmup_step_size"], dtype=np.float64)
        sampling_step_size = np.asarray(archive["sampler_sampling_step_size"], dtype=np.float64)
        ...
        warmup_step_size_finite_positive = bool(
            np.all(np.isfinite(warmup_step_size)) and np.all(warmup_step_size > 0.0)
        )
        warmup_step_size_initial_match = bool(
            warmup_step_size_finite_positive
            and np.array_equal(
        ...
        sampling_step_size_finite_positive = bool(
            sampling_step_size.shape == (4,)
            and np.all(np.isfinite(sampling_step_size))
            and np.all(sampling_step_size > 0.0)
        )
        ...
        if sampling_is_divergent.shape != expected_sampling_shape or sampling_energy.shape != expected_sampling_shape:
            raise SamplerMechanicsV4Error("v4 sampling trace shape mismatch")
        ...
        if not isinstance(sampler, dict) or not isinstance(checks, dict):
            raise SamplerMechanicsV4Error("v4 task sampler/check records are missing")
        if not np.array_equal(
            np.asarray(sampler.get("warmup_divergence_count"), dtype=int),
            warmup_divergence_count,
        ):
            raise SamplerMechanicsV4Error("v4 warmup divergence count does not match NPZ")
        if float(sampler.get("warmup_divergence_fraction")) != warmup_divergence_fraction:
            raise SamplerMechanicsV4Error("v4 warmup divergence fraction does not match NPZ")
        if not np.array_equal(
            np.asarray(sampler.get("warmup_divergence_fraction_by_block"), dtype=np.float64),
            warmup_divergence_fraction_by_block,
        ):
            raise SamplerMechanicsV4Error("v4 warmup block divergence trace does not match NPZ")
        if not np.array_equal(
            np.asarray(sampler.get("warmup_energy_finite_by_block"), dtype=bool),
            warmup_energy_finite_by_block,
        ):
            raise SamplerMechanicsV4Error("v4 warmup block energy trace does not match NPZ")
        if not np.array_equal(
            ...
            raise SamplerMechanicsV4Error("v4 initialization trace does not match NPZ")
        if not warmup_step_size_initial_match:
            raise SamplerMechanicsV4Error("v4 warmup first-used step size does not match frozen initial step")
        if not np.array_equal(
            np.asarray(sampler.get("warmup_step_size_first_used"), dtype=np.float64),
            warmup_step_size[:, 0],
        ) or not np.array_equal(
            np.asarray(sampler.get("warmup_step_size_last_used"), dtype=np.float64),
            warmup_step_size[:, -1],
        ):
            raise SamplerMechanicsV4Error("v4 warmup step-size endpoints do not match NPZ")
        if not np.array_equal(
            np.asarray(sampler.get("step_size"), dtype=np.float64), sampling_step_size
        ) or not np.array_equal(
            np.asarray(sampler.get("sampling_step_size"), dtype=np.float64), sampling_step_size
        ):
            raise SamplerMechanicsV4Error("v4 sampling step size does not match NPZ")
        if sampler.get("warmup_step_size_trace_stored") is not (
            warmup_step_size_finite_positive and warmup_step_size_initial_match
        ):
            raise SamplerMechanicsV4Error("v4 warmup step-size trace gate does not match NPZ")
        if checks.get("warmup_step_size_trace_stored") is not (
            warmup_step_size_finite_positive and warmup_step_size_initial_match
        ):
            raise SamplerMechanicsV4Error("v4 warmup step-size check does not match NPZ")
        if sampler.get("sampling_step_size_trace_stored") is not sampling_step_size_finite_positive:
            raise SamplerMechanicsV4Error("v4 sampling step-size trace gate does not match NPZ")
        if checks.get("sampling_step_size_trace_stored") is not sampling_step_size_finite_positive:
            raise SamplerMechanicsV4Error("v4 sampling step-size check does not match NPZ")
        if checks.get("all_warmup_energies_finite") is not warmup_energy_finite:
            raise SamplerMechanicsV4Error("v4 warmup energy finiteness gate does not match NPZ")
        if checks.get("warmup_divergence_fraction") is not (
            warmup_divergence_fraction <= 0.01
        ):
            raise SamplerMechanicsV4Error("v4 warmup divergence gate does not match NPZ")
        if float(sampler.get("sampling_divergence_fraction")) != sampling_divergence_fraction:
            raise SamplerMechanicsV4Error("v4 sampling divergence fraction does not match NPZ")
        if checks.get("divergence_fraction") is not (
            sampling_divergence_fraction <= 0.01
        ):
            raise SamplerMechanicsV4Error("v4 sampling divergence gate does not match NPZ")
        if checks.get("all_sampling_energies_finite") is not sampling_energy_finite:
            raise SamplerMechanicsV4Error("v4 sampling energy finiteness gate does not match NPZ")
        if sampler.get("warmup_energy_trace_stored") is not True or checks.get(
            "warmup_energy_trace_stored"
        ) is not True:
            raise SamplerMechanicsV4Error("v4 warmup trace storage gate is not true")
        if checks.get("warmup_energy_trace_stored") is not True:
            raise SamplerMechanicsV4Error("v4 warmup trace storage check is inconsistent")
        if bool(checks) and bool(result.get("pilot_pass")) != bool(all(checks.values())):
            raise SamplerMechanicsV4Error("v4 pilot pass does not match task checks")
    if result["checks"].get("warmup_energy_trace_stored") is not True:
        raise SamplerMechanicsV4Error("v4 warmup energy trace storage gate failed")
    return result
...
def aggregate(
    ...
    if len(implementation_commit) != 40:
        raise SamplerMechanicsV4Error("v4 aggregate commit must be full hash")
    implementation_sha = sha256_file(__file__)
    output = Path(aggregate_output)
    staging = output.parent / f".{output.name}.staging"
    if output.exists() or staging.exists():
        raise SamplerMechanicsV4Error("v4 aggregate output already exists")
    outcomes = []
    for assignment in controller["assignments"]:
        ...
        try:
            ...
            if result.get("assignment") != assignment:
                raise SamplerMechanicsV4Error("v4 task assignment mismatch")
            if result.get("program", {}).get("sha256") != program_sha:
                raise SamplerMechanicsV4Error("v4 task program hash mismatch")
            implementation = result.get("implementation", {})
            if implementation.get("path") != str(Path(__file__).resolve()):
                raise SamplerMechanicsV4Error("v4 task implementation path mismatch")
            if implementation.get("sha256") != implementation_sha:
                raise SamplerMechanicsV4Error("v4 task implementation hash mismatch")
            if implementation.get("commit") != implementation_commit:
                raise SamplerMechanicsV4Error("v4 task implementation commit mismatch")
            outcomes.append(
                {
                    "assignment": assignment,
    ...
    result = {
        ...
        "status": "PASS_BOTH_SAMPLER_MECHANICS_V4_TASKS" if aggregate_pass else "NO_GO_SAMPLER_MECHANICS_V4_STOP",
        "program": {"path": str(Path(program_path).resolve()), "sha256": program_sha},
    ...
    (staging / "manifest.json").write_text(
        json.dumps(artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v4-aggregate-manifest-v1"), indent=2, sort_keys=True)
        + "\n"
    )
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v4-aggregate-complete-v1",
...
def validate_aggregate(directory: str | Path) -> dict[str, object]:
    ...
    if not root.is_dir() or {path.name for path in root.iterdir()} != AGGREGATE_FILES:
        raise SamplerMechanicsV4Error("v4 aggregate artifact set mismatch")
    result = json.loads((root / "aggregate.json").read_text())
    if result.get("schema") != AGGREGATE_SCHEMA or result.get("task_count") != 2:
        raise SamplerMechanicsV4Error("v4 aggregate schema mismatch")
    complete = json.loads((root / "COMPLETE").read_text())
    if complete.get("aggregate_sha256") != sha256_file(root / "aggregate.json"):
        raise SamplerMechanicsV4Error("v4 aggregate hash mismatch")
    if complete.get("manifest_sha256") != sha256_file(root / "manifest.json"):
        raise SamplerMechanicsV4Error("v4 aggregate manifest hash mismatch")
    if complete.get("both_pilot_tasks_pass") != result.get("both_pilot_tasks_pass"):
        raise SamplerMechanicsV4Error("v4 aggregate decision mismatch")
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v4-aggregate-manifest-v1")
    return result
