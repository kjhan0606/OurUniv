#!/usr/bin/env python
"""Create and verify the committed V18/Astrid one-shot artifact seal."""
...
from hong2021_v14_edm import V15_E3_SCHEMA, decoder_upsampling_for_schema
...
SCHEMA = "hong2021-v18-astrid-one-shot-artifact-seal-v1"
...
DEFAULT_DESTINATION = Path("config/hong2021_v18_astrid_one_shot_seal.json")
EXACT_ONE_SHOT_COMMAND = (
    "scripts/run_hong2021_v18_astrid_one_shot_lageunha.sh "
    "config/hong2021_v18_astrid_one_shot_seal.json"
)
...
def _selected_artifacts(
    ...
    sequence_path = tng / "evaluation/tng100_simba_swift_v18_sequence/status.json"
    sequence = _load_json(sequence_path)
    if sequence.get("state") != "complete_e6_passed_astrid_still_unopened":
        raise RuntimeError("V18 has no completed passing three-domain development gate")
    decision_path = Path(sequence["detail"]).resolve()
    decision = _load_json(decision_path)
    if decision.get("experiment") != "e6_prior_matched_initialization":
        raise RuntimeError("V18 sequence refers to the wrong experiment")
    if decision.get("development_pass") is not True:
        raise RuntimeError("V18 development decision is not passing")
    if decision.get("next") != "freeze_exact_v18_hashes_before_astrid_one_shot":
        raise RuntimeError("V18 decision does not authorize the Astrid seal")
    if canonical_digest(decision) != decision.get("decision_digest_sha256"):
        raise RuntimeError("V18 development decision digest is invalid")
    if decision.get("Astrid_used") is not False:
        raise RuntimeError("V18 decision does not prove Astrid remained unused")
    registry_path = repo / "config/hong2021_v18_development_program.json"
    registry = load_frozen_registry(registry_path, repo)
    if decision.get("registry_sha256") != FROZEN_REGISTRY_SHA256:
        raise RuntimeError("V18 decision used a different frozen registry")
    ...
    if Path(decision["selected_checkpoint"]).resolve() != checkpoint_path:
        raise RuntimeError("V18 selected checkpoint differs from the E3 registry")
    selected = next(
        row for row in decision["candidates"] if int(row["step"]) == selected_step
    )
    if selected.get("checkpoint_sha256") != checkpoint_row["sha256"]:
        raise RuntimeError("V18 selected checkpoint hash differs from the E3 registry")
    ...
    provenance = {
        "v18_sequence": sequence_path,
        "development_decision": decision_path,
        "v18_registry": registry_path,
        "initialization_measurement": Path(
            registry["e6_prior_matched_initialization"]["initialization"][
                "measurement_report"
            ]
        ),
        "v15_registry": repo / registry["parent_evidence"]["v15_registry"],
        "e3_decision": Path(registry["parent_evidence"]["e3_decision"]),
...
def create_seal(
    *, repo: Path, tng: Path, astrid_root: Path, destination: Path
    ...
    if astrid_files(astrid_root):
        raise RuntimeError("Astrid must remain unopened before the V18 seal commit")
    ...
    if artifact_rows["edm"]["sha256"] != selection["checkpoint_row"]["sha256"]:
        raise RuntimeError("selected E3 checkpoint changed after the V18 gate")
    ...
    if edm.get("schema") != V15_E3_SCHEMA:
        raise ValueError("selected EDM is not the frozen V15-E3 model")
    if decoder_upsampling_for_schema(edm["schema"]) != "nearest":
        raise ValueError("selected E3 model does not use nearest decoding")
    if edm.get("experiment_registry_sha256") != selection["registry"][
        "parent_evidence"
    ]["v15_registry_sha256"]:
        raise ValueError("selected E3 model has the wrong registry provenance")
    ...
    experiment = selection["registry"]["e6_prior_matched_initialization"]
    ...
    record = {
        ...
        "model_invariants": {
            ...
            "edm_p_mean_sigma_data_fraction": float(
                edm["edm_p_mean_sigma_data_fraction"]
            ),
            "tail_exponent": float(
        ...
        "initialization": {
            ...
            "inference_initialization_mapping": mapping,
        ...
        "one_shot": {
            ...
            "rho": 7.0,
            "seed": 28777,
            "initializer": "sealed V18 prior-matched 80^3 physical-k mapping",
        ...
        "commit_requirement": "This record must be the only file in a commit directly atop the frozen V18 code before any Astrid file is downloaded or opened.",
    }
...
def verify_seal(
    ...
    *,
    ...
    if record.get("schema") != SCHEMA:
        raise ValueError("not a V18 Astrid one-shot seal")
    ...
    if invariants.get("edm_schema") != V15_E3_SCHEMA:
        raise RuntimeError("seal does not retain the frozen E3 checkpoint")
    ...
    if init.get("registry_sha256") != FROZEN_REGISTRY_SHA256:
        raise RuntimeError("seal does not retain the frozen V18 registry")
    ...
    mapping = init.get("inference_initialization_mapping", {})
    if (
        ...
        raise RuntimeError("seal does not retain the zero-parameter 80^3 mapping")
    if _run_git(repo, "status", "--porcelain"):
    ...
    if require_committed:
        ...
        if len(commits) != 1:
            raise RuntimeError("the V18 seal must be added once and never modified")
        seal_commit = commits[0]
        if _run_git(repo, "rev-parse", f"{seal_commit}^") != code_commit:
            raise RuntimeError("the seal commit is not directly atop the frozen V18 code")
        changed = _run_git(
            repo, "diff-tree", "--no-commit-id", "--name-only", "-r", seal_commit
        ).splitlines()
        if changed != [relative]:
            raise RuntimeError("the seal commit must contain only the V18 seal record")
        ...
        if committed != path.read_bytes():
            raise RuntimeError("the working V18 seal differs from its committed blob")
    ...
    if record["one_shot"]["command"] != EXACT_ONE_SHOT_COMMAND:
        raise RuntimeError("V18 one-shot command differs from the verifier")
    if require_unopened and astrid_files(Path(record["astrid_preopen"]["root"])):
        raise RuntimeError("Astrid was already opened before V18 seal verification")
    return record
