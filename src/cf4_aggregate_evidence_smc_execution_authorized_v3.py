...
CANONICAL_PROGRAM = (
    ROOT / "config/cf4_aggregate_evidence_smc_execution_authorization_program_v3.json"
)
PROGRAM_SHA256 = "c899cff7b98038d5823d91623adc874c9699a0784fdb8c6718873a01e6883a2f"
CANONICAL_DESIGN = (
    ROOT / "config/cf4_aggregate_evidence_smc_execution_authorization_design_v3.json"
)
DESIGN_SHA256 = "62be0352ffb9c180ba300894169a1503d46109011b100f3e860dc2621f858112"
GRANT_RELATIVE_PATH = "config/cf4_aggregate_evidence_smc_execution_grant_v3.json"
...
DATA_DIRECTORY = Path(
    "/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v3"
)
STATE_DIRECTORY = Path(
    "/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v3_run"
)
...
AUTHORIZATION_V2_PATHS = (
    "config/cf4_aggregate_evidence_smc_execution_authorization_design_v3.json",
    "config/cf4_aggregate_evidence_smc_execution_authorization_program_v3.json",
    "src/cf4_aggregate_evidence_smc_execution_authorized_v3.py",
    "tests/test_cf4_aggregate_evidence_smc_execution_authorized_v3.py",
    "scripts/run_cf4_aggregate_evidence_smc_authorized_v3_lageunha.sh",
    "scripts/launch_cf4_aggregate_evidence_smc_authorized_v3_lageunha.sh",
    "tests/test_cf4_aggregate_evidence_smc_authorized_v3_runner.py",
)
...
GRANT_KEYS = {
    ...
    "authorization_v3_implementation_commit",
    "authorization_v3_implementation_result_record",
    "authorization_v3_files",
    "data_directory",
    "state_directory",
    "authorization",
...
def _validate_design() -> None:
    ...
    if design.get("schema") != (
        "ouruniv-cf4-aggregate-evidence-smc-execution-authorization-design-v3"
...
def validate_authorization_program(
    ...
    if program.get("schema") != (
        "ouruniv-cf4-aggregate-evidence-smc-execution-authorization-program-v3"
    ...
    if grant != {
        ...
        "schema": "ouruniv-cf4-aggregate-evidence-smc-execution-grant-v3",
        ...
        "grant_parent_commit_source": (
            "The commit containing the immutable authorization-v3 "
            "implementation result record."
        ),
        "authorization_v3_implementation_result_record_path": (
            AUTHORIZATION_RESULT_RELATIVE_PATH
        ),
        "prospective_values_resolved_from_grant_parent_not_self_pinned": True,
        "exact_commit_chain": (
            "grant_parent^ == authorization_v3_implementation_commit and "
...
def _validate_result_record(grant: dict[str, Any], *, parent: str) -> str:
    binding = grant.get("authorization_v3_implementation_result_record")
    if not isinstance(binding, dict) or set(binding) != {"path", "sha256"} \
            or binding.get("path") != AUTHORIZATION_RESULT_RELATIVE_PATH:
        raise RuntimeError("authorization-v3 implementation result binding changed")
    expected_sha = _require_sha256(
        binding.get("sha256"), "authorization-v3 implementation result record"
    )
    ...
    if not record_path.is_file() or record_path.read_bytes() != tracked_record \
        ...
        raise RuntimeError("authorization-v3 implementation result record changed")
    record = json.loads(tracked_record)
    implementation_commit = _require_commit(
        grant.get("authorization_v3_implementation_commit"),
        "authorization-v3 implementation commit",
    )
    ...
    if implementation_parent != implementation_commit:
        raise RuntimeError(
            "grant parent is not the direct child of the authorization-v3 "
    ...
    if record.get("schema") != (
        ...
        "complete_pass_postcommit_authorization_v3_implementation"
    ) or record.get("commit_lineage", {}).get("git_commit") != (
        implementation_commit
    ):
        raise RuntimeError("authorization-v3 implementation result lineage changed")
    rows = record.get("committed_authorization_files")
    if grant.get("authorization_v3_files") != rows \
            or not isinstance(rows, list) or len(rows) != 7 \
            or [row.get("path") for row in rows] != list(AUTHORIZATION_V2_PATHS):
        raise RuntimeError("authorization-v3 seven-file binding changed")
    for row in rows:
        if not isinstance(row, dict) or set(row) != {
            "path", "sha256", "git_blob_oid", "git_mode"
        }:
            raise RuntimeError("authorization-v3 file record schema changed")
        relative = row["path"]
        ...
        if len(blob) != 40 \
            ...
            raise RuntimeError(f"authorization-v3 file changed: {relative}")
    return expected_sha
...
def _validate_external_lineage_release(
    ...
    expected_keys = {
        ...
        "grant_sha256", "authorization_v3_implementation_result_record_sha256",
    }
    if set(release) != expected_keys \
            ...
            or release.get(
                "authorization_v3_implementation_result_record_sha256"
            ) != result_sha:
...
def validate_future_grant(
    ...
    if set(grant) != GRANT_KEYS or grant.get("schema") != (
            "ouruniv-cf4-aggregate-evidence-smc-execution-grant-v3"
...
def run_authorized_v3(program_path: Path) -> dict[str, Any]:
    ...
    if path != CANONICAL_PROGRAM.resolve():
        raise PermissionError("authorized v3 accepts only the canonical program path")
...
def main() -> None:
    ...
    result = run_authorized_v3(arguments.program)
    print(
