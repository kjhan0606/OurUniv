...
PROGRAM = ROOT / "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_authorized_program_v6.json"
GRANT = ROOT / "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_execution_grant_v6.json"
RELEASE = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_execution_release_v6.json")
MANIFEST = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_execution_manifest_v6.json")
RECEIPT_ROOT = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_receipts_v6")
...
def _validate_program(program_file: SnapshotFile) -> dict[str, Any]:
    ...
    if (
        program.get("schema") != "ouruniv-cf4-v6-open-parent-key-overlap-canonical-read-only-analysis-authorized-program-v6"
        or program.get("status") != "frozen_v6_program_execution_false_until_sealed_v6_pair_grant_and_audits"
        or program.get("date") != "2026-09-03"
        or program.get("purpose") != "Freeze the executable v6 binding for the exact path-aware one-shot memory-only parent/key overlap analysis."
    ):
    ...
    expected_resource = {"cpus_per_task": 4, "login_host": "syntax", "node": "syn06", "nodes": 1, "ntasks": 1, "partition": "a40", "pre_registered_maximum_expected_RSS_GiB": 6.5, "requested_memory_GiB": 8, "requested_memory_margin_over_expected_percent": 23.076923076923077, "requeue": False, "submission_mechanism": "Slurm_only", "time_limit": "01:00:00"}
    ...
    expected_order = (
        ("src/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_authorized_v6.py", "0644"),
        ("scripts/run_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_authorized_v6.sbatch", "0755"),
        ("scripts/status_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_authorized_v6.sh", "0755"),
        ("tests/test_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_authorized_v6.py", "0644"),
        ("tests/test_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_parent_key_overlap_canonical_read_only_analysis_authorized_v6_runner.py", "0644"),
    )
...
def _validate_pair(program_file: SnapshotFile, grant_file: SnapshotFile, release_file: SnapshotFile, manifest_file: SnapshotFile, program: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (
        grant.get("schema") != "ouruniv-cf4-v6-open-parent-key-overlap-canonical-read-only-analysis-execution-grant-v6"
    ...
    if (
        release.get("schema") != "ouruniv-cf4-v6-open-parent-key-overlap-canonical-read-only-analysis-execution-release-v6"
        or release.get("status") != "complete_pass_external_postcommit_lineage_audit"
    ):
        raise AuthorizationError("release identity changed")
    if (
        manifest.get("schema") != "ouruniv-cf4-v6-open-parent-key-overlap-canonical-read-only-analysis-execution-manifest-v6"
    ...
    if (
        payload.get("schema") != "ouruniv-cf4-v6-open-parent-key-overlap-canonical-read-only-analysis-execution-release-payload-v6"
    ...
    release_id = hashlib.sha256(
        ("ouruniv-parent-key-overlap-release-v6\0" + PAIR_DESIGN_COMMIT + "\0" + REMEDIATION_DESIGN_SHA + "\0" + program_file.sha256).encode()
    ).hexdigest()
    manifest_id = hashlib.sha256(
        ("ouruniv-parent-key-overlap-manifest-v6\0" + PAIR_DESIGN_COMMIT + "\0" + release_id).encode()
    ).hexdigest()
    grant_id = hashlib.sha256(
        ("ouruniv-parent-key-overlap-grant-v6\0" + PAIR_DESIGN_COMMIT + "\0" + release_file.sha256 + "\0" + manifest_file.sha256 + "\0" + program_file.sha256).encode()
    ).hexdigest()
...
def _validate_host_and_allocation(hostname: str, environment: Mapping[str, str]) -> None:
    if _ascii_short_host(hostname) != EXPECTED_HOST:
        raise AuthorizationError("execution host is not syn06")
    expected = {"SLURM_JOB_NUM_NODES": "1", "SLURM_NTASKS": "1", "SLURM_CPUS_PER_TASK": "4", "SLURM_MEM_PER_NODE": "8192", "SLURM_JOB_PARTITION": "a40", "SLURM_JOB_NAME": "cf4-parent-overlap-v1"}
...
def _validate_marker(name: str, value: Mapping[str, Any], *, bundle: AuthorizationBundle | None = None, snapshot_sha: str | None = None) -> None:
    ...
    contracts = {
        "RUNNING": (
            ...
            "ouruniv-cf4-v6-open-parent-key-overlap-read-only-analysis-receipt-running-v6",
            "running_authorized_read_only_in_memory_analysis",
        ),
        "COMPLETE": (
            common | {"schema", "status", "analysis_return_validated_in_process"},
            "ouruniv-cf4-v6-open-parent-key-overlap-read-only-analysis-receipt-complete-v6",
            "complete_valid_provenance_and_in_memory_analysis_postvalidation",
        ),
        "FAILED": (
            common | {"schema", "status", "failure_class", "checkpoint"},
            "ouruniv-cf4-v6-open-parent-key-overlap-read-only-analysis-receipt-failed-v6",
    ...
    try:
        with _masked_signals():
          try:
            ...
            hook("after_snapshot")
            _assert_lease_chain(lease)
            running = {"schema": "ouruniv-cf4-v6-open-parent-key-overlap-read-only-analysis-receipt-running-v6", "status": "running_authorized_read_only_in_memory_analysis", "grant_id": bundle.grant["grant_id"], "release_id": bundle.release["release_id"], "manifest_id": bundle.manifest["manifest_id"], "snapshot_sha256": snapshot_sha}
...
def _seal_failed_at(bundle: AuthorizationBundle, item: CreatedDirectory, snapshot_sha: str, *, lease: ReceiptLease | None = None, checkpoint: str, require_canonical_name: bool) -> None:
    ...
    try:
        ...
        pass
    value = {"schema": "ouruniv-cf4-v6-open-parent-key-overlap-read-only-analysis-receipt-failed-v6", "status": "failed_invalid_provenance_read_or_analysis_execution", **_marker_common(bundle, snapshot_sha), "failure_class": "invalid_provenance_read_or_analysis_execution", "checkpoint": checkpoint}
...
def _seal_complete(lease: ReceiptLease) -> None:
    with _masked_signals():
        ...
        value = {"schema": "ouruniv-cf4-v6-open-parent-key-overlap-read-only-analysis-receipt-complete-v6", "status": "complete_valid_provenance_and_in_memory_analysis_postvalidation", **_marker_common(lease.bundle, lease.snapshot_sha), "analysis_return_validated_in_process": True}
...
def _valid_analysis_result(value: Any) -> bool:
    """Validate the exact immutable analyzer return frozen by the v6 design."""
...
def _run_authorized_for_test(*, git_runner: Callable[..., subprocess.CompletedProcess[bytes]], hostname: str, environment: Mapping[str, str], loader_factory: Callable[[Mapping[str, Any]], Mapping[str, Any]] | None = None, interrupt: Callable[[str], None] | None = None) -> Mapping[str, Any]:
    ...
    try:
        ...
        if not _valid_analysis_result(result):
            raise AuthorizationError("analysis return violates the exact v6 immutable schema")
        _revalidate_snapshot(lease.bundle, lease.item, lease.snapshot_sha, lease=lease)
...
def run_authorized_canonical_parent_key_overlap_read_only_analysis_v6() -> Mapping[str, Any]:
...
def main() -> int:
    ...
    if mode is None and child is None:
        run_authorized_canonical_parent_key_overlap_read_only_analysis_v6()
        return 0
