#!/usr/bin/env python3
"""Mock-only sampler-mechanics repair after the audited L=12 ACF failure."""
...
SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-pilot-v2"
TASK_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v2-task-v1"
AGGREGATE_SCHEMA = "ouruniv-cf4-phasec-sampler-mechanics-v2-aggregate-v1"
TASK_FILES = {"diagnostics.npz", "result.json", "manifest.json", "COMPLETE"}
AGGREGATE_FILES = {"aggregate.json", "manifest.json", "COMPLETE"}


class SamplerMechanicsV2Error(ValueError):
...
def load_program(path: str | Path) -> tuple[dict[str, object], str, dict[str, object]]:
    ...
    try:
        ...
        raise SamplerMechanicsV2Error("cannot parse v2 sampler program") from exc
    if controller.get("schema") != SCHEMA:
        raise SamplerMechanicsV2Error("v2 sampler schema mismatch")
    auth = controller.get("authorization", {})
    for key in (
        "replacement_Phase_C_mock_sampler_mechanics_v2",
        ...
        if auth.get(key) is not True:
            raise SamplerMechanicsV2Error(f"missing v2 authorization: {key}")
    for key in (
        ...
        if auth.get(key) is not False:
            raise SamplerMechanicsV2Error(f"forbidden v2 scope enabled: {key}")
    if controller.get("assignments") != expected_assignments():
        raise SamplerMechanicsV2Error("v2 sampler assignments changed")
    expected_lineage = {
        "base_program",
        "replacement_program",
    ...
    if set(lineage) != expected_lineage:
        raise SamplerMechanicsV2Error("v2 lineage set changed")
    for name, binding in lineage.items():
        ...
        if expected_hash.startswith("TO_BE_FILLED"):
            raise SamplerMechanicsV2Error("v2 implementation is not frozen")
        if not source_path.is_file() or sha256_file(source_path) != expected_hash:
            raise SamplerMechanicsV2Error(f"v2 lineage mismatch: {name}")

    mechanics = controller.get("mechanics", {})
    if mechanics.get("latent_dimension") != 32792:
        raise SamplerMechanicsV2Error("v2 latent dimension changed")
    expected_map = {
    ...
    if mechanics.get("MAP") != expected_map:
        raise SamplerMechanicsV2Error("v2 MAP mechanics changed")
    expected_sampler = {
    ...
    if mechanics.get("sampler") != expected_sampler:
        raise SamplerMechanicsV2Error("v2 sampler mechanics changed")
    if mechanics.get("field_probe_indices") != [0, 1, 31, 32, 1024, 4096, 16384, 32767]:
        raise SamplerMechanicsV2Error("v2 field probes changed")
    expected_gates = {
    ...
    if controller.get("gates") != expected_gates:
        raise SamplerMechanicsV2Error("v2 sampler gates changed")
    if controller.get("truth_integrator") != {
        ...
        raise SamplerMechanicsV2Error("v2 truth integrator changed")
    ...
    if (
        ...
        raise SamplerMechanicsV2Error("v2 hardware quarantine changed")

    base_path = Path(controller["lineage"]["base_program"]["path"])
    base = json.loads(base_path.read_text())
    if base.get("schema") != v1.phasec_v5.SCHEMA:
        raise SamplerMechanicsV2Error("base mock schema mismatch")
    for binding in base.get("input_bindings", {}).values():
        ...
        if not input_path.is_file() or sha256_file(input_path) != binding["sha256"]:
            raise SamplerMechanicsV2Error(f"base mock input mismatch: {input_path}")
    ...
    if base.get("mock_assignments") != expected_base_assignments:
        raise SamplerMechanicsV2Error("base mock assignments changed")
    for key in (
        "actual_observational_field_inference",
        "actual_2Mpp_count_read",
        "actual_CF4_velocity_datum_used",
        ...
        if base.get("authorization", {}).get(key) is not False:
            raise SamplerMechanicsV2Error(f"base mock forbidden scope enabled: {key}")
    return controller, hashlib.sha256(payload).hexdigest(), copy.deepcopy(base)


def task_name(assignment: Mapping[str, object]) -> str:
    return (
        f"mechanics_v2_{int(assignment['task_index']):02d}_mock_{int(assignment['mock_index']):02d}"
...
def validate_manifest(directory: Path, schema: str) -> None:
    ...
    if manifest.get("schema") != schema:
        raise SamplerMechanicsV2Error("v2 manifest schema mismatch")
...
def run_identity_hmc(
    ...
    if not math.isfinite(map_value) or not np.all(np.isfinite(map_gradient)):
        raise SamplerMechanicsV2Error("v2 MAP value or gradient is nonfinite")
    map_gradient_rms = float(np.linalg.norm(map_gradient) / math.sqrt(map_gradient.size))
    if not np.all(np.isfinite(map_position)):
        raise SamplerMechanicsV2Error("v2 MAP position is nonfinite")
    ...
    for chain in range(chain_count):
        ...
        if not np.all(np.isfinite(positions_np)):
            raise SamplerMechanicsV2Error(f"v2 chain {chain} contains nonfinite positions")
...
def run_task(
    ...
    if task_index not in (0, 1) or len(implementation_commit) != 40:
        raise SamplerMechanicsV2Error("invalid v2 task index or commit")
    assignment = controller["assignments"][task_index]
    output = Path(output_root) / task_name(assignment)
    staging = output.parent / f".{output.name}.staging"
    if output.exists() or staging.exists():
        raise SamplerMechanicsV2Error("v2 output or staging already exists")
    ...
    if jax.default_backend() != "gpu" or len(jax.devices()) != 1:
        raise SamplerMechanicsV2Error("v2 task requires one Slurm GPU")
    ...
    result = {
        "schema": TASK_SCHEMA,
        "status": "PASS_SAMPLER_MECHANICS_V2_TASK" if pilot_pass else "NO_GO_SAMPLER_MECHANICS_V2_TASK",
    ...
    (staging / "manifest.json").write_text(
        json.dumps(artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v2-task-manifest-v1"), indent=2, sort_keys=True)
        + "\n"
    )
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v2-task-complete-v1",
...
def validate_task(directory: str | Path) -> dict[str, object]:
    ...
    if not root.is_dir() or {path.name for path in root.iterdir()} != TASK_FILES:
        raise SamplerMechanicsV2Error("v2 task artifact set mismatch")
    result = json.loads((root / "result.json").read_text())
    if result.get("schema") != TASK_SCHEMA or not isinstance(result.get("pilot_pass"), bool):
        raise SamplerMechanicsV2Error("v2 task result schema mismatch")
    complete = json.loads((root / "COMPLETE").read_text())
    if complete.get("result_sha256") != sha256_file(root / "result.json"):
        raise SamplerMechanicsV2Error("v2 task result hash mismatch")
    if complete.get("manifest_sha256") != sha256_file(root / "manifest.json"):
        raise SamplerMechanicsV2Error("v2 task manifest hash mismatch")
    if complete.get("pilot_pass") != result["pilot_pass"]:
        raise SamplerMechanicsV2Error("v2 task decision mismatch")
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v2-task-manifest-v1")
    ...
    with np.load(root / "diagnostics.npz", allow_pickle=False) as archive:
        ...
        if not required.issubset(archive.files):
            raise SamplerMechanicsV2Error("v2 warmup traces are missing")
        if any(np.asarray(archive[name]).shape != expected_warmup_shape for name in required if name != "sampler_warmup_divergence_count"):
            raise SamplerMechanicsV2Error("v2 warmup trace shape mismatch")
        if np.asarray(archive["sampler_warmup_divergence_count"]).shape != (4,):
            raise SamplerMechanicsV2Error("v2 warmup count shape mismatch")
    if result["checks"].get("warmup_energy_trace_stored") is not True:
        raise SamplerMechanicsV2Error("v2 warmup energy trace storage gate failed")
    return result
...
def aggregate(
    ...
    if len(implementation_commit) != 40:
        raise SamplerMechanicsV2Error("v2 aggregate commit must be full hash")
    output = Path(aggregate_output)
    staging = output.parent / f".{output.name}.staging"
    if output.exists() or staging.exists():
        raise SamplerMechanicsV2Error("v2 aggregate output already exists")
    outcomes = []
    for assignment in controller["assignments"]:
        ...
        try:
            ...
            if result.get("assignment") != assignment:
                raise SamplerMechanicsV2Error("v2 task assignment mismatch")
            outcomes.append(
                {
                    "assignment": assignment,
    ...
    result = {
        ...
        "status": "PASS_BOTH_SAMPLER_MECHANICS_V2_TASKS" if aggregate_pass else "NO_GO_SAMPLER_MECHANICS_V2_STOP",
        "program": {"path": str(Path(program_path).resolve()), "sha256": program_sha},
    ...
    (staging / "manifest.json").write_text(
        json.dumps(artifact_manifest(staging, "ouruniv-cf4-phasec-sampler-mechanics-v2-aggregate-manifest-v1"), indent=2, sort_keys=True)
        + "\n"
    )
    complete = {
        "schema": "ouruniv-cf4-phasec-sampler-mechanics-v2-aggregate-complete-v1",
...
def validate_aggregate(directory: str | Path) -> dict[str, object]:
    ...
    if not root.is_dir() or {path.name for path in root.iterdir()} != AGGREGATE_FILES:
        raise SamplerMechanicsV2Error("v2 aggregate artifact set mismatch")
    result = json.loads((root / "aggregate.json").read_text())
    if result.get("schema") != AGGREGATE_SCHEMA or result.get("task_count") != 2:
        raise SamplerMechanicsV2Error("v2 aggregate schema mismatch")
    complete = json.loads((root / "COMPLETE").read_text())
    if complete.get("aggregate_sha256") != sha256_file(root / "aggregate.json"):
        raise SamplerMechanicsV2Error("v2 aggregate hash mismatch")
    if complete.get("manifest_sha256") != sha256_file(root / "manifest.json"):
        raise SamplerMechanicsV2Error("v2 aggregate manifest hash mismatch")
    if complete.get("both_pilot_tasks_pass") != result.get("both_pilot_tasks_pass"):
        raise SamplerMechanicsV2Error("v2 aggregate decision mismatch")
    validate_manifest(root, "ouruniv-cf4-phasec-sampler-mechanics-v2-aggregate-manifest-v1")
    return result
