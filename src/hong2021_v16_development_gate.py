#!/usr/bin/env python
"""Integrity-bound three-domain development gate for V16-E4."""
...
from hong2021_v16_edm import load_frozen_registry


SCHEMA = "hong2021-v16-integrity-bound-three-domain-decision-v1"
POWER_BANDS = ("0.3-1_h_mpc", "1-3_h_mpc", "3-6_h_mpc", "6-10.0531_h_mpc")
FROZEN_DEVELOPMENT_OBJECT_SHA256 = {
    "config/hong2021_simba_dev_representative16_v1.json": (
        "8e60a07328f75058a682ffd42a4667655f62935baaeb3985b10bcdd727a92663"
    ),
    "config/hong2021_swift_eagle_dev_representative16_v1.json": (
        "d0475680dc846332b928833961a34802fb4560dd100f8714bc95241db13582b7"
    ),
}
...
def _validate_checkpoint(
    ...
    if checkpoint.get("schema") != V16_E4_SCHEMA:
        raise ValueError("unexpected V16-E4 checkpoint schema")
    if int(checkpoint.get("step", -1)) != step:
        raise ValueError("checkpoint step differs from candidate step")
    if checkpoint.get("decoder_upsampling") != "trilinear":
        raise ValueError("V16-E4 checkpoint did not use trilinear decoder upsampling")
    if checkpoint.get("decoder_align_corners") is not False:
        raise ValueError("V16-E4 checkpoint did not freeze align_corners=false")
    if checkpoint.get("edm_p_mean_mode") != "log_sigma_data_fraction":
        raise ValueError("V16-E4 checkpoint did not retain relative noise placement")
    if float(checkpoint.get("edm_p_mean_sigma_data_fraction", -1)) != 0.6:
        raise ValueError("V16-E4 checkpoint used the wrong sigma_data fraction")
    if float(checkpoint.get("edm_p_std", -1)) != 1.2:
        raise ValueError("V16-E4 checkpoint used the wrong p_std")
    tail = checkpoint.get("tail_weight_fit", {})
    if float(tail.get("inverse_probability_exponent", -1)) != 0.25:
        raise ValueError("V16-E4 checkpoint did not retain the E3 tail exponent")
    if float(tail.get("pre_normalization_maximum", -1)) != 10.0:
        raise ValueError("V16-E4 checkpoint did not retain the E3 tail maximum")
...
def _all_band_report(metrics: dict[str, Any]) -> dict[str, float]:
    power = metrics["fourier_log_density"]["generated_total_power_over_truth"]
    return {band: float(power[band]) for band in POWER_BANDS}


def _load_expected_indices(
    specification: list[int] | str, repo: Path
) -> list[int]:
    if isinstance(specification, list):
        ...
        if not path.is_absolute():
            path = repo / path
        expected_sha = FROZEN_DEVELOPMENT_OBJECT_SHA256.get(specification)
        if expected_sha is None or sha256_file(path) != expected_sha:
            raise ValueError("V16 development subset file differs from its frozen hash")
        indices = [int(value) for value in json.loads(path.read_text())["indices"]]
    if len(indices) != 16 or len(set(indices)) != 16 or min(indices) < 0:
        raise ValueError("V16 development subset must contain 16 unique indices")
    return indices
...
def _validate_source_indices(path: Path, expected: list[int]) -> None:
    ...
    if actual != expected:
        raise ValueError("ensemble source indices differ from the frozen V16 subset")


def _next_after_failure(final: dict[str, Any]) -> str:
    domains = final["domains"]
    tng = domains["tng"]
    tng_metrics = json.loads(Path(tng["metrics"]).read_text())["candidates"]["edm"]
    two_point = tng_metrics["two_point_cosmic_mean"]
    generated = two_point["generated_vs_truth_ks"]["by_scale"]["0-1_mpc_h"]["mean"]
    deterministic = two_point["deterministic_vs_truth_ks"]["by_scale"]["0-1_mpc_h"]["mean"]
    if generated >= deterministic:
        return "stop_unopened_then_predeclare_band_balanced_denoising_loss"
    ...
    only_swift_peak_void = (
        ...
        and all(
            value
            for name, value in swift_checks.items()
...
def evaluate(
    *, root: Path, training: Path, registry_path: Path, repo: Path,
    ...
    if steps != [5000, 10000]:
        raise ValueError("candidate steps differ from the V16 registry")
    seed_map = {
    ...
    expected_indices = {
        "tng": _load_expected_indices(object_specifications["TNG100"], repo),
        "simba_dev": _load_expected_indices(object_specifications["SIMBA"], repo),
        "swift_dev": _load_expected_indices(object_specifications["Swift"], repo),
    }
    candidates = []
    for step in steps:
        ...
        for domain in DOMAINS:
            ...
            _validate_ensemble(
                ...
                checkpoint_schema=V16_E4_SCHEMA,
                step=step,
            ...
            domains[domain] = {
                ...
                "report_only_total_power_over_truth": _all_band_report(metrics),
            }
        candidates.append(
            {
                "step": step,
    ...
    report = {
        ...
        "next": (
            "freeze_exact_v16_hashes_before_astrid_one_shot"
...
def main() -> None:
    ...
    if not clean:
        raise RuntimeError("V16 gate requires a clean committed worktree")
    report = evaluate(
        root=args.root,
    ...
    if args.out.exists():
        ...
        if canonical_digest(existing) != existing.get("decision_digest_sha256"):
            raise RuntimeError("existing V16 decision has an invalid digest")
        if existing != report:
            raise RuntimeError("existing V16 decision differs from recomputed decision")
