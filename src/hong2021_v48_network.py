#!/usr/bin/env python
"""V48 identifiable query-local Gaussian-mixture U-Net and likelihood."""
...
BISECTION_STEPS = 28
RANK_EPSILON = 1.0e-7
GAUSSIAN_TAIL_Z = 5.199337582290662
INITIAL_LOCATIONS = (
    -1.2815515655446004,
    -0.5244005127080409,
    0.0,
...
INITIAL_SCALE = 0.48275446642024056
INITIAL_RAW_SCALE = -0.503506807232178
INITIAL_BIASES = (
    (0.0,) * MIXTURES
    + INITIAL_LOCATIONS
...
class LocalMixtureUNet(nn.Module):
    """The architecture and identifiable initialization frozen for V48."""
    ...
    def forward(self, condition: torch.Tensor) -> torch.Tensor:
        if condition.ndim != 5 or condition.shape[1] != INPUT_CHANNELS:
            raise ValueError("V48 condition shape differs")
...
def mixture_parameters(
    ...
    if parameters.ndim != 5 or parameters.shape[1] != OUTPUT_CHANNELS:
        raise ValueError("V48 mixture parameter shape differs")
...
def gaussian_mixture_log_probability(
    ...
    if value.shape != (len(parameters), 1, *parameters.shape[-3:]):
        raise ValueError("V48 likelihood target shape differs")
...
def gaussian_mixture_inverse(
    ...
    *,
    ...
    if uniform.shape != (len(parameters), 1, *parameters.shape[-3:]):
        raise ValueError("V48 mixture rank shape differs")
    probability = uniform.float().clamp(RANK_EPSILON, 1.0 - RANK_EPSILON)
    lower = torch.min(
        locations - GAUSSIAN_TAIL_Z * scales, dim=1, keepdim=True
    ).values
    upper = torch.max(
        locations + GAUSSIAN_TAIL_Z * scales, dim=1, keepdim=True
    ).values
