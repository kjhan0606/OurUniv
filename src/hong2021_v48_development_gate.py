#!/usr/bin/env python
"""Integrity-bound three-domain development gate for V48."""
...
from hong2021_v48_sample import ARMS, ENSEMBLE_SCHEMA
from hong2021_v48_train import PREFLIGHT_SCHEMA, PROGRAM_SHA256, _verified_json, load_program


SCHEMA = "hong2021-v48-identifiable-query-local-Gaussian-mixture-copula-three-domain-decision-v1"
HIGH_K_RMS_CHECKS = (
...
def _validate(
    ...
    with h5py.File(path, "r") as current, h5py.File(parent, "r") as old:
        exact = {
            ...
            "method": "train_only_identifiable_query_local_gaussian_mixture_empirical_rank_copula",
            "arm": arm,
            "v48_program_sha256": PROGRAM_SHA256,
        ...
        for key, expected in exact.items():
            if _value(current.attrs.get(key)) != expected:
                raise ValueError(f"V48 {domain} {arm} metadata differs: {key}")
        reused = (
            "source_index",
            "donor_source",
            "donor_index",
        ...
        if tuple(current["sample"].shape) != (16, 16, 1, 64, 64, 64) or any(
            ...
            raise ValueError("V48 ensemble shape or frozen selection differs")
        ...
        if maximum_dc > 1.0e-7 or inverse_error > 2.0e-6:
            raise ValueError("V48 residual DC or inverse CDF differs")
        ...
        if (
            ...
            raise ValueError("V48 artifact hash differs")
        ...
        if (
            ...
            or not checked.get("finite_physical_Gaussian_moment_proof")
        ...
        if subprocess.run(
            ...
            raise ValueError("V48 sampling commit is not an ancestor")
...
def classify(
    ...
    if primary:
        return (
            "identifiable_query_local_gaussian_mixture_copula_sufficient",
            "seal_v48_and_await_explicit_approval_before_independent_gate",
        )
    if q3 and q4:
        return (
            "finite_moment_local_marginal_is_calibrated_but_empirical_rank_copula_limits_morphology",
            "audit_only_conditional_rank_copula_dependence_without_changing_the_Gaussian_marginal_likelihood",
        )
    if high_k_rms:
        return (
            "finite_moment_gaussian_mixture_body_is_supported_but_extreme_calibration_is_insufficient",
            "audit_train_only_Gaussian_mixture_PIT_extreme_quantiles_and_physical_moments_without_threshold_tuning",
        )
    if rolled_improves:
        return (
...
def evaluate(root: Path, program_path: Path, repo: Path, commit: str) -> dict[str, Any]:
    ...
    for arm in ARMS:
        ...
        for domain in DOMAIN_ORDER:
            ...
            if Path(metrics["path"]).resolve() != ensemble.resolve():
                raise ValueError("V48 metrics point elsewhere")
            domains[domain] = {
    ...
    for domain in DOMAIN_ORDER:
        reference = internal["identifiable_query_local_gaussian_mixture_copula"][domain]
        for arm in ARMS[1:]:
            ...
            if (
                ...
                raise ValueError("V48 arms changed donor ranks or object amplitude")
    candidate = arms["identifiable_query_local_gaussian_mixture_copula"]
    ...
    result: dict[str, Any] = {
        "schema": SCHEMA,
        "experiment": "v48_identifiable_query_local_gaussian_mixture_copula",
...
def main() -> None:
    ...
    if not clean:
        raise RuntimeError("V48 gate requires a clean committed worktree")
    result = evaluate(args.root.resolve(), args.program.resolve(), args.repo.resolve(), commit)
    if args.out.exists():
        raise FileExistsError("V48 refuses existing decision")
