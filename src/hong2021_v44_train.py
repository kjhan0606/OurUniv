#!/usr/bin/env python
"""Prepare, preflight, and train the frozen V44 local mixture likelihood."""
...
from hong2021_v41_two_stage import (
    FIT_REPORT_SCHEMA as V41_REPORT_SCHEMA,
    FIT_SCHEMA as V41_SCHEMA,
    _predictions,
...
PROGRAM_SCHEMA = "hong2021-v44-query-local-mixture-copula-development-program-v1"
PROGRAM_SHA256 = "5929bb84b47cc21db09a281feecb802a6831e96ffc858c479fe73ef51b21fadb"
CACHE_SCHEMA = "hong2021-v44-train-only-conditioning-cache-v1"
PREFLIGHT_SCHEMA = "hong2021-v44-local-mixture-copula-hard-preflight-v1"
CHECKPOINT_SCHEMA = "hong2021-v44-local-mixture-copula-checkpoint-v1"
REPORT_SCHEMA = "hong2021-v44-local-mixture-copula-training-report-v1"
STEPS = 12_000
...
def load_program(
    ...
    program = _verified_json(path.resolve(), PROGRAM_SHA256, "V44 program")
    if (
        ...
        raise ValueError("V44 program schema or status differs")
    parent = program["parent_evidence"]
    record = _verified_json(
        (repo / parent["v43_record"]).resolve(),
        parent["v43_record_sha256"],
        "V44 V43 record",
    )
    ...
    if (
        ...
        raise ValueError("V44 V43 conclusion or firewall differs")
    ...
    if sha256_file(v35_path) != inherited["v35_program_sha256"]:
        raise ValueError("V44 V35 program hash differs")
    v35, _ = load_v35_program(v35_path, repo)
    if (
        sha256_file((repo / inherited["v31_record"]).resolve())
        != inherited["v31_record_sha256"]
        or sha256_file(Path(inherited["conditional_copula_artifact"]))
        != inherited["conditional_copula_artifact_sha256"]
        or sha256_file(Path(inherited["v41_artifact"]))
        != inherited["v41_artifact_sha256"]
        or sha256_file(Path(inherited["v41_report"]))
        != inherited["v41_report_sha256"]
    ):
        raise ValueError("V44 inherited artifact hash differs")
    v41_report = json.loads(Path(inherited["v41_report"]).read_text())
    v41 = joblib.load(inherited["v41_artifact"])
    if (
        v41_report.get("schema") != V41_REPORT_SCHEMA
        or v41_report.get("validation_opened") is not False
        or v41.get("schema") != V41_SCHEMA
        or v41.get("seed_block_count") != 11
    ):
        raise ValueError("V44 V41 model metadata differs")
    return program, v35, v41
...
def prepare_cache(
    ...
    if not clean:
        raise RuntimeError("V44 cache preparation requires a clean worktree")
    if output.exists() or report_path.exists():
        raise FileExistsError("V44 refuses existing conditioning cache outputs")
    ...
    with h5py.File(partial, "w") as prepared:
                ...
                try:
                    ...
                    for index in range(objects):
                        ...
                        if (index + 1) % 32 == 0 or index + 1 == objects:
                            print(
                                f"[v44-cache] {domain} {split} {index + 1}/{objects}",
                                flush=True,
                            )
                finally:
                    data.close()
        ...
        if (
            ...
            raise RuntimeError("V44 train-only standardization differs")
    ...
    result: dict[str, Any] = {
        "schema": "hong2021-v44-conditioning-cache-report-v1",
...
def load_cache(path: Path, digest: str, commit: str) -> h5py.File:
    if sha256_file(path) != digest:
        raise ValueError("V44 conditioning cache hash differs")
    ...
    if (
        ...
        handle.close()
        raise ValueError("V44 conditioning cache metadata differs")
    return handle
...
def preflight(
    ...
    commit, clean = git_state(repo.resolve())
    if not clean:
        raise RuntimeError("V44 preflight requires a clean worktree")
    device = _device()
    ...
    try:
        ...
        for domain in DOMAIN_ORDER:
            ...
            if not np.array_equal(np.sort(condition.reshape(-1)), np.sort(restored_condition.reshape(-1))):
                raise RuntimeError("V44 isometry coverage differs")
        ...
        if parameter_count(model) != PARAMETERS:
            raise RuntimeError("V44 parameter count differs")
        ...
        if torch.count_nonzero(parameters).item() != 0:
            raise RuntimeError("V44 final initialization is not exactly zero")
        ...
        if not torch.isfinite(loss) or not gradients or not all(torch.isfinite(g).all() for g in gradients):
            raise RuntimeError("V44 real-data loss or gradients differ")
        if not any(torch.count_nonzero(g).item() for g in gradients):
            raise RuntimeError("V44 gradients are all zero")
        ...
        if cdf_error > 2.0e-6 or peak >= 24 * 1024**3:
            raise RuntimeError("V44 mixture inverse or memory preflight differs")
        ...
        try:
            ...
            if not np.isfinite(sample).all() or real_dc > 1.0e-7:
                raise RuntimeError("V44 real paired-donor sample differs")
        finally:
            query_data.close()
            query_cache.close()
            donor_data.close()
    ...
    result["decision_digest_sha256"] = canonical_digest(result)
    if output.exists():
        raise FileExistsError("V44 refuses existing preflight")
    output.parent.mkdir(parents=True, exist_ok=True)
    partial = output.with_suffix(output.suffix + ".partial")
...
def train(
    ...
    commit, clean = git_state(repo.resolve())
    if not clean:
        raise RuntimeError("V44 training requires a clean worktree")
    if checkpoint_path.exists() or report_path.exists():
        raise FileExistsError("V44 refuses existing training outputs")
    checked = _verified_json(preflight_path, preflight_sha, "V44 preflight")
    if (
    ...
    try:
        ...
        for step in range(1, STEPS + 1):
            ...
            if step == 1 or step % 50 == 0:
                ...
                log_rows.append(row)
                print("[v44-train] " + json.dumps(row), flush=True)
            if step in VALIDATION_STEPS:
                validation_rows[str(step)] = _validation_nll(
                    ema_model, v35, prepared, device
                )
                print(
                    f"[v44-validation] step={step} {json.dumps(validation_rows[str(step)])}",
                    flush=True,
                )
    finally:
