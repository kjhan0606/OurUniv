#!/usr/bin/env python
"""Create and verify the committed V15/Astrid one-shot artifact seal."""
...
SCHEMA = "hong2021-v15-astrid-one-shot-artifact-seal-v1"
...
DEFAULT_DESTINATION = Path("config/hong2021_v15_astrid_one_shot_seal.json")
EXACT_ONE_SHOT_COMMAND = (
    "scripts/run_hong2021_v15_astrid_one_shot_lageunha.sh "
    "config/hong2021_v15_astrid_one_shot_seal.json"
)
EXPERIMENT_SCHEMA = {"e2": V15_E2_SCHEMA, "e3": V15_E3_SCHEMA}


def _selected_artifacts(tng: Path) -> tuple[dict[str, Path], dict[str, Path], dict[str, Any]]:
    sequence_path = tng / "evaluation/tng100_simba_swift_v15_sequence/status.json"
    sequence = _load_json(sequence_path)
    expected_states = {
        "complete_e2_passed_astrid_still_unopened": "e2",
        "complete_e3_passed_astrid_still_unopened": "e3",
    }
    if sequence.get("state") not in expected_states:
        raise RuntimeError("V15 has no completed passing three-domain development gate")
    experiment = expected_states[sequence["state"]]
    decision_path = Path(sequence["detail"]).resolve()
    decision = _load_json(decision_path)
    if decision.get("experiment") != experiment or decision.get("development_pass") is not True:
        raise RuntimeError("V15 sequence and development decision disagree")
    if canonical_digest(decision) != decision.get("decision_digest_sha256"):
        raise RuntimeError("V15 development decision digest is invalid")
    if decision.get("Astrid_used") is not False:
        raise RuntimeError("V15 decision does not prove Astrid remained unused")
    ...
    if correction_selection.get("development_pass") is not True:
        raise RuntimeError("frozen mean correction did not pass its development selection")
    ...
    provenance = {
        "v15_sequence": sequence_path,
        "development_decision": decision_path,
        "v15_registry": DEFAULT_REPO / "config/hong2021_v15_development_program.json",
        "mean_correction_selection": correction_selection_path,
        "model_preparation_status": preparation_path,
    }
    return artifacts, provenance, {"experiment": experiment, "decision": decision}
...
def create_seal(
    *, repo: Path, tng: Path, astrid_root: Path, destination: Path
    ...
    if astrid_files(astrid_root):
        raise RuntimeError("Astrid must remain unopened before the V15 seal commit")
    artifacts, provenance, selection = _selected_artifacts(tng.resolve())
    ...
    if edm.get("schema") != EXPERIMENT_SCHEMA[experiment]:
        raise ValueError("selected EDM schema differs from the passing experiment")
    ...
    for label, commit in (
        ...
        ("gate", selection["decision"]["gate_code_commit"]),
    ):
    ...
    record = {
        ...
        "development_gate": {
            ...
            "gate_code_commit": selection["decision"]["gate_code_commit"],
            "decision_digest_sha256": selection["decision"]["decision_digest_sha256"],
        ...
        "commit_requirement": "This record must be the only file in a commit directly atop the frozen V15 code before any Astrid file is downloaded or opened.",
    }
...
def verify_seal(
    ...
    *,
    ...
    if record.get("schema") != SCHEMA:
        raise ValueError("not a V15 Astrid one-shot seal")
    ...
    if require_committed:
        ...
        if len(seal_commits) != 1:
            raise RuntimeError("the V15 seal must be added once and never modified")
        ...
        if committed != path.read_bytes():
            raise RuntimeError("the working V15 seal differs from its committed blob")
    ...
    if record["one_shot"]["command"] != EXACT_ONE_SHOT_COMMAND:
        raise RuntimeError("V15 one-shot command differs from the verifier")
    if require_unopened and astrid_files(Path(record["astrid_preopen"]["root"])):
        raise RuntimeError("Astrid was already opened before V15 seal verification")
    return record
