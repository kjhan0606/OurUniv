#!/usr/bin/env python
"""Preflight helpers and fixed training loop for the V83 marginal spline flow."""
...
CHECKPOINT_SCHEMA = "hong2021-v83-conditional-marginal-spline-checkpoint-v1"
REPORT_SCHEMA = "hong2021-v83-conditional-marginal-spline-training-report-v1"
STEPS = 12_000
SEED = 830144
EMA_DECAY = 0.999
INITIAL_LR = 2.0e-4
...
def seeded_model(device: torch.device) -> ConditionalMarginalSplineUNet:
    previous = torch.random.get_rng_state()
    torch.manual_seed(SEED)
    model = ConditionalMarginalSplineUNet().to(device)
...
def _load_authorized_inputs(
...
) -> tuple[dict[str, Any], dict[str, Any], dict[str, dict[str, list[int]]], str]:
    commit, clean = git_state(repo)
    if not clean:
        raise RuntimeError("V83 training requires a clean frozen worktree")
    ...
    if (
        ...
        != "hong2021-v83-conditional-marginal-spline-hard-preflight-v1"
        or preflight.get("status") != "pass"
        or preflight.get("program_sha256") != sha256_file(program_path)
        or preflight.get("training_performed") is not False
        or preflight.get("validation_payload_accessed") is not False
        or preflight.get("independent_gate_locked") is not True
    ):
        raise ValueError("V83 preflight authorization differs")
    return program, v35, partition, commit
...
def train(
    ...
    repo = repo.resolve()
    if (
        socket.gethostname().split(".")[0].lower() != "lageunha"
        or not torch.cuda.is_available()
        or "ada" not in torch.cuda.get_device_name(0).lower()
    ):
        raise RuntimeError("V83 training requires the Lageunha Ada GPU")
    if checkpoint_path.exists() or report_path.exists():
        raise FileExistsError("V83 training refuses existing outputs")
    program, v35, partition, commit = _load_authorized_inputs(
        program_path,
        repo,
        conditioning_cache,
        cache_sha256,
    ...
    if (
        ...
        or program["training"]["objective"] != "proper_conditional_negative_log_likelihood_only"
    ):
        raise ValueError("V83 frozen training contract differs")
    ...
    if parameter_count(model) != expected_parameters:
        raise RuntimeError("V83 model parameter count differs")
    ...
    try:
        ...
        for step in range(1, STEPS + 1):
            ...
            if step == 1 or step % 50 == 0:
                ...
                log_rows.append(row)
                print("[v83-train] " + json.dumps(row), flush=True)
    finally:
    ...
    common = {
        ...
        "objective": "proper_conditional_negative_log_likelihood_only",
        ...
        "posthoc_scale_clipping_DC_or_Ak": False,
    }
    checkpoint = {
    ...
    result: dict[str, Any] = {
        ...
        "status": "complete_fixed_12000_step_proper_conditional_spline_fit",
        **common,
