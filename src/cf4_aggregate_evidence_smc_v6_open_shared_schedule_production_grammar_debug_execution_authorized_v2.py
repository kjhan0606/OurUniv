...
PROGRAM = ROOT / "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_program_v2.json"
GRANT_RELATIVE = "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_grant_v2.json"
WRAPPER_RESULT_RECORD_RELATIVE = "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_implementation_result_record_v2.json"
WRAPPER_DESIGN = ROOT / "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_design_v2.json"
V1_PROGRAM = ROOT / "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_program_v1.json"
...
RELEASE = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_release_v2.json")
EXTERNAL_MANIFEST = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_manifest_v2.json")
RECEIPT_ROOT = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_receipts")
CACHE_ROOT = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_cache")
DATA_ROOT = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2")
STATE_ROOT = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_run")
RUNNER = ROOT / "scripts/run_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2.sbatch"
SLURM_LOG_TEMPLATE = Path("/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2_slurm-%j.log")
PROC_SELF_CGROUP = Path("/proc/self/cgroup")
PROC_SELF_MOUNTINFO = Path("/proc/self/mountinfo")

WRAPPER_DESIGN_SHA256 = "c7b8375641307655f53a9fcae8c0d142c1cf66425cf188fa58a9b3a137bce37a"
IMPLEMENTATION_RECORD_SHA256 = "57ccc0ac99ead2a3903e96e22f0b40182e2a6db3a3b3184a5257d8d8530ba867"
WRAPPER_DESIGN_COMMIT = "b24b494633d91d991bdb67850b1a6d8cf2f6d9ad"
BRANCH = "agent/freeze-zoom-pipeline"
REMOTE_REF = f"refs/heads/{BRANCH}"
IMPLEMENTATION_FILES = (
    "config/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_program_v2.json",
    "src/cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_authorized_v2.py",
    "scripts/run_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2.sbatch",
    "scripts/status_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2.sh",
    "tests/test_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_execution_authorized_v2.py",
    "tests/test_cf4_aggregate_evidence_smc_v6_open_shared_schedule_production_grammar_debug_v2.py",
)
...
SNAPSHOT_KEYS = {
    ...
    "requeue", "minimum_memory_node", "req_tres_raw", "req_tres_parsed",
    "alloc_tres_raw", "alloc_tres_parsed", "cgroup_version",
    "cgroup_controller", "cgroup_value_path", "cgroup_value_raw",
}
RUNNING_SCHEDULER_KEYS = {
    "SLURM_JOB_ID", "combined_log_path", "combined_log_pre_receipt_dev",
    "combined_log_pre_receipt_ino", "combined_log_pre_receipt_size_bytes",
    "combined_log_pre_receipt_sha256_of_exact_prefix_size_bytes",
    "req_tres_raw", "req_tres_parsed", "alloc_tres_raw", "alloc_tres_parsed",
...
def load_program() -> dict[str, Any]:
    ...
    if set(program) != PROGRAM_KEYS \
            or program.get("schema") != "ouruniv-cf4-v6-open-shared-schedule-production-grammar-debug-execution-program-v2" \
    ...
    v1_program_row = design.get("immutable_v1_chain", {}).get("implementation_files", {}).get("program", {})
    _require_file(V1_PROGRAM, v1_program_row.get("sha256", ""), "0644", "grammar-debug v1 program")
    v1_program = _read_json(V1_PROGRAM, "grammar-debug v1 program")
    fixed_science = program.get("fixed_science")
    if fixed_science != v1_program.get("fixed_science"):
        raise AuthorizationError("fixed science differs from design")
    design_science = {key: value for key, value in fixed_science.items() if key != "reuse_forbidden"}
    if design.get("science_contract") != {
        "bitwise_identical_to_v1": True,
        **design_science,
        "all_v1_science_pins_and_gates_remain_exact": True,
    }:
        raise AuthorizationError("v2 science identity is not the exact v1 contract")
    expected_paths = {
    ...
    pins = v1_program["lineage"]["protected_science_hard_pins"]
    ...
    surface = design.get("implementation_surface", {})
    if surface.get("exact_files") != list(IMPLEMENTATION_FILES) \
            or surface.get("modes") != ["0644", "0644", "0755", "0755", "0644", "0644"] \
            or surface.get("semantic_delta_exact_allowlist") != [
                "versioned_v2_program_source_sbatch_status_test_paths",
                "v2_pair_grant_runtime_and_schema_identifiers",
                "TRES_to_ReqTRES_AllocTRES_parser_replacement",
                "scheduler_snapshot_and_marker_fields_for_raw_and_parsed_ReqTRES_AllocTRES",
                "v2_lineage_and_hash_pins",
            ] \
            or surface.get("all_other_v1_science_lifecycle_receipt_signal_timeout_artifact_postcheck_semantics_exact") is not True:
        raise AuthorizationError("v2 semantic delta allowlist changed")
    ...
    wanted_resource = {
        ...
        "maximum_expected_RSS_GiB": 80, "requested_memory_GiB": 96,
        "memory_margin_fraction": 0.2,
        "cgroup_memory_limit_minimum_bytes": 103079215104,
        "cgroup_membership_source": "/proc/self/cgroup",
        "cgroup_mount_source": "/proc/self/mountinfo",
...
def _scontrol_job(job_id: int) -> dict[str, str]:
    ...
    for token in output.split():
        if "=" not in token:
            raise AuthorizationError("scontrol output contains a malformed field")
        key, value = token.split("=", 1)
        if not key or key in result:
            raise AuthorizationError("scontrol output contains a duplicate field")
        result[key] = value
    return result
...
def _parse_tres(value: str, label: str, allowed: set[str]) -> dict[str, str]:
    if not value or value.strip() != value:
        raise AuthorizationError(f"{label} is empty or contains outer whitespace")
    result: dict[str, str] = {}
    for token in value.split(","):
        if not token or token.count("=") != 1:
            raise AuthorizationError(f"{label} contains a malformed entry")
        key, item = token.split("=", 1)
        if not key or not item or key not in allowed or key in result:
            raise AuthorizationError(f"{label} contains an unknown or duplicate key")
        result[key] = item
    return result
...
def _stable_regular_bytes(path: Path, label: str) -> bytes:
    path = Path(path)
    if path.is_symlink():
        raise AuthorizationError(f"{label} final target is a symlink")

    def read_once() -> tuple[bytes, tuple[int, int, int]]:
        flags = os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0)
        try:
            descriptor = os.open(path, flags)
            try:
                ...
                if not stat.S_ISREG(before.st_mode):
                    raise AuthorizationError(f"{label} is not a regular file")
                chunks: list[bytes] = []
                total = 0
                while True:
                    ...
                    total += len(block)
                    if total > 8 << 20:
                        raise AuthorizationError(f"{label} is unexpectedly large")
                    chunks.append(block)
                after = os.fstat(descriptor)
            finally:
                os.close(descriptor)
        except OSError as error:
            raise AuthorizationError(f"{label} cannot be read safely") from error
    ...
    if first != second or first_identity != second_identity:
        raise AuthorizationError(f"{label} is unstable across reads")
    return first


def _proc_text(path: Path, label: str) -> str:
    try:
        return _stable_regular_bytes(path, label).decode("utf-8")
    except UnicodeDecodeError as error:
        raise AuthorizationError(f"{label} is not UTF-8") from error
...
def _unescape_mount_field(value: str) -> str:
    ...
    if "\\" in value:
        raise AuthorizationError("mountinfo contains an unsupported escape")
    return value


def _safe_absolute_posix(value: str, label: str) -> PurePosixPath:
    path = PurePosixPath(value)
    if not path.is_absolute() or ".." in path.parts:
        raise AuthorizationError(f"{label} is not a canonical absolute path")
    return path


def _cgroup_value_target(mount: str, root: str, membership: str, filename: str) -> Path:
    mount_path = _safe_absolute_posix(mount, "cgroup mount")
    root_path = _safe_absolute_posix(root, "cgroup mount root")
    membership_path = _safe_absolute_posix(membership, "cgroup membership")
    try:
        relative = membership_path.relative_to(root_path)
    except ValueError as error:
        raise AuthorizationError("cgroup membership is outside its mount root") from error
    try:
        canonical_mount = Path(str(mount_path)).resolve(strict=True)
        target = Path(str(mount_path.joinpath(relative, filename)))
        canonical_parent = target.parent.resolve(strict=True)
    except OSError as error:
        raise AuthorizationError("cgroup target cannot be resolved") from error
    if canonical_parent != canonical_mount and canonical_mount not in canonical_parent.parents:
        raise AuthorizationError("cgroup target escapes the canonical mount")
    if target.is_symlink():
        raise AuthorizationError("cgroup value final target is a symlink")
    return target


def _resolve_memory_cgroup() -> dict[str, str]:
    membership_text = _proc_text(PROC_SELF_CGROUP, "process cgroup membership")
    mount_text = _proc_text(PROC_SELF_MOUNTINFO, "process mountinfo")
    unified: list[str] = []
    memory_memberships: list[str] = []
    seen_controllers: set[str] = set()
    for line in membership_text.splitlines():
        parts = line.split(":", 2)
        if len(parts) != 3:
            raise AuthorizationError("cgroup membership line is malformed")
        hierarchy, raw_controllers, member = parts
        _safe_absolute_posix(member, "cgroup membership")
        controllers = raw_controllers.split(",") if raw_controllers else []
        if any(not item or item in seen_controllers for item in controllers):
            raise AuthorizationError("cgroup controller membership is duplicate or malformed")
        seen_controllers.update(controllers)
        if hierarchy == "0" and not controllers:
            unified.append(member)
        if "memory" in controllers:
            memory_memberships.append(member)

    mounts: list[tuple[str, str, str, set[str]]] = []
    for line in mount_text.splitlines():
        if " - " not in line:
            raise AuthorizationError("mountinfo separator is absent")
        left_raw, right_raw = line.split(" - ", 1)
        left = left_raw.split()
        right = right_raw.split()
        if len(left) < 6 or len(right) < 3:
            raise AuthorizationError("mountinfo line is malformed")
        root = _unescape_mount_field(left[3])
        mount = _unescape_mount_field(left[4])
        _safe_absolute_posix(root, "cgroup mount root")
        _safe_absolute_posix(mount, "cgroup mount")
        mounts.append((right[0], root, mount, set(right[2].split(","))))

    if len(memory_memberships) == 1:
        candidates = [row for row in mounts if row[0] == "cgroup" and "memory" in row[3]]
        if len(candidates) != 1:
            raise AuthorizationError("cgroup v1 memory mount is absent or ambiguous")
        _, root, mount, _ = candidates[0]
        target = _cgroup_value_target(mount, root, memory_memberships[0], "memory.limit_in_bytes")
        version, controller = "v1", "memory"
    elif len(unified) == 1 and not memory_memberships:
        candidates = [row for row in mounts if row[0] == "cgroup2"]
        if len(candidates) != 1:
            raise AuthorizationError("cgroup v2 mount is absent or ambiguous")
        _, root, mount, _ = candidates[0]
        target = _cgroup_value_target(mount, root, unified[0], "memory.max")
        version, controller = "v2", "unified"
    else:
        raise AuthorizationError("memory cgroup version or membership is ambiguous")

    try:
        raw = _stable_regular_bytes(target, "memory cgroup limit").decode("ascii").strip()
    except UnicodeDecodeError as error:
        raise AuthorizationError("memory cgroup limit is not ASCII") from error
    if version == "v2" and raw == "max":
        pass
    elif not raw.isdecimal() or int(raw) < 103079215104:
        raise AuthorizationError("memory cgroup limit is below 96 GiB or malformed")
    return {
        "cgroup_version": version, "cgroup_controller": controller,
        "cgroup_value_path": str(target), "cgroup_value_raw": raw,
    }
...
def _read_scheduler_context() -> dict[str, Any]:
    ...
    request_raw = str(details.get("ReqTRES", ""))
    allocation_raw = str(details.get("AllocTRES", ""))
    request = _parse_tres(request_raw, "ReqTRES", {"cpu", "mem", "node", "billing"})
    allocation = _parse_tres(allocation_raw, "AllocTRES", {"cpu", "mem", "node", "billing"})
    expected = {
    ...
    if any(details.get(key) != value for key, value in expected.items()) \
            ...
            or request != {"cpu": "12", "mem": "96G", "node": "1", "billing": "12"} \
            or any(allocation.get(key) != value for key, value in {
                "cpu": "12", "node": "1", "billing": "12",
            }.items()) \
            or set(allocation) not in ({"cpu", "node", "billing"}, {"cpu", "mem", "node", "billing"}) \
            or ("mem" in allocation and allocation["mem"] != "96G"):
        raise AuthorizationError("scontrol allocation provenance changed")
    cgroup = _resolve_memory_cgroup()
    ...
    return {
        ...
        "minimum_memory_node": minimum_memory,
        "req_tres_raw": request_raw, "req_tres_parsed": request,
        "alloc_tres_raw": allocation_raw, "alloc_tres_parsed": allocation,
        **cgroup,
        "combined_log": log,
    }
...
def _snapshot(
    ...
    snapshot = {
        "schema": "ouruniv-cf4-v6-open-shared-schedule-production-grammar-debug-receipt-snapshot-v2",
        ...
        "minimum_memory_node": scheduler["minimum_memory_node"],
        "req_tres_raw": scheduler["req_tres_raw"],
        "req_tres_parsed": scheduler["req_tres_parsed"],
        "alloc_tres_raw": scheduler["alloc_tres_raw"],
        "alloc_tres_parsed": scheduler["alloc_tres_parsed"],
        "cgroup_version": scheduler["cgroup_version"],
        "cgroup_controller": scheduler["cgroup_controller"],
        "cgroup_value_path": scheduler["cgroup_value_path"],
        "cgroup_value_raw": scheduler["cgroup_value_raw"],
...
def _receipt_failed(
    ...
    with _blocked_lifecycle_signals():
        ...
        if snapshot_path.is_file() and not snapshot_path.is_symlink():
            ...
            terminal_scheduler = {}
        failed = {
            "schema": "ouruniv-cf4-v6-open-shared-schedule-production-grammar-debug-receipt-marker-v2",
...
def _scheduler_from_snapshot(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "minimum_memory_node": snapshot["minimum_memory_node"],
        "req_tres_raw": snapshot["req_tres_raw"],
        "req_tres_parsed": snapshot["req_tres_parsed"],
        "alloc_tres_raw": snapshot["alloc_tres_raw"],
        "alloc_tres_parsed": snapshot["alloc_tres_parsed"],
        "cgroup_version": snapshot["cgroup_version"],
        "cgroup_controller": snapshot["cgroup_controller"],
        "cgroup_value_path": snapshot["cgroup_value_path"],
        "cgroup_value_raw": snapshot["cgroup_value_raw"],
        "combined_log": {
...
def _read_only_complete_status() -> dict[str, Any]:
    ...
    if set(state_value) != {
        ...
        "release_id", "manifest_id", "result_manifest_sha256_or_null", "science_status",
            ...
            or state_value.get("schema") != "ouruniv-cf4-v6-open-shared-schedule-production-grammar-debug-state-marker-v2" \
    ...
    if set(receipt_value) != {
        ...
        "snapshot_sha256", "result_manifest_sha256_or_null",
            ...
            or receipt_value.get("schema") != "ouruniv-cf4-v6-open-shared-schedule-production-grammar-debug-receipt-marker-v2" \
...
def _read_only_running_status() -> dict[str, Any]:
    ...
    if set(value) != {
        "schema", "status", "grant_id", "release_id", "manifest_id", "snapshot_sha256",
            ...
            or value.get("schema") != "ouruniv-cf4-v6-open-shared-schedule-production-grammar-debug-state-marker-v2" \
    ...
    if set(receipt_running) != {
        "schema", "status", "grant_id", "release_id", "manifest_id", "snapshot_sha256",
            ...
            or receipt_running.get("schema") != "ouruniv-cf4-v6-open-shared-schedule-production-grammar-debug-receipt-marker-v2" \
            or any(
...
def _require_resources(scheduler: Mapping[str, Any]) -> None:
    try:
        available_kib = next(
    ...
    cgroup = _resolve_memory_cgroup()
    if any(scheduler.get(key) != value for key, value in cgroup.items()):
        raise AuthorizationError("memory cgroup identity changed after scheduler validation")
    raw_limit = cgroup["cgroup_value_raw"]
    unlimited = raw_limit == "max" or (
        cgroup["cgroup_version"] == "v1" and raw_limit.isdecimal()
        and int(raw_limit) >= 1 << 60
    )
    if unlimited and (
        scheduler.get("req_tres_parsed") != {
            "cpu": "12", "mem": "96G", "node": "1", "billing": "12",
        }
        or scheduler.get("memory_MiB") != 98304
    ):
        raise AuthorizationError("unlimited cgroup is not backed by the exact scheduler allocation")
