...
Fable5 plan audit completed normally in121178ms: CONDITIONAL GO. Essential
conditions are incorporated below; disposition BUNDLE_C_DIFFUSION_AUDIT.md.
The exact submitted plan and original response are preserved separately.
...
batch1, EMA.999 and training seed902101; no heldout tuning. Exact transition
...
Commit its complete versioned branch table with the implementation. Run the
native-code identity test before the FIRST optimizer update in the same Slurm
allocation, saving its result. No login-node calculation or separate audit job.
Report canonicalization counts per scale/channel, not only a global average;
frequent corrections expose a learned/support mismatch even if sums conserve.
...
The mathematical sampler includes an explicit failure outcome for numerical
decoding failures. Conditioning on success without its normalizer is NOT the
unchanged field law. Each invalid fixed evaluation draw counts as a morphology
failure, with scale and reason retained; no replacement draw or dropped row.
...
At the cap, report completed updates and learning trend. If completed updates
are below30,000, the result status is INCONCLUSIVE_BUDGET (unless an earlier
numerical/operational error supplies its own failure status). Partial quality
and trend remain reported evidence about that finite run, but cannot promote
the model or establish diffusion-family success/failure.
...
If this attempt fails morphology or is inconclusive, the next proposed science
investment must address member-state learnability/identifiability using native
fields BEFORE any further field-model training. No automatic extension of this
run or serial architecture trials. This sequencing condition does not authorize
that next study now, resurrect closed peak/proxy/budget tests, or claim a finite
negative probe proves mathematical non-identifiability. Its concrete plan must
test the missing same-field joint-member information, not native-label copying.
...
Before submission save one compact config sizing record with actual model
parameter/state counts, activation/modalities/buffer/cache estimates, predicted
update time range and evaluation allowance. Keep predicted versus measured
values distinct. The32-update segment measures worst-scale forward/backward
and optimizer memory against that record; its updates count toward the fit.
If sizing changes the approved compute cap or architecture materially, report
and request direction rather than silently changing the experiment.
