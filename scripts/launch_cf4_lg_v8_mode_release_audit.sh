...
readonly session=cf4-v8-mode-audit
readonly runner=/home/kjhan/BACKUP/CF4/scripts/run_cf4_lg_v8_mode_release_audit.sh
...
if [[ -e "$state/result.json" || -e "$state/RUNNING" \
    ...
    echo "audit output or lifecycle file already exists under $state" >&2
    exit 73
fi
if [[ ! -x "$runner" ]]; then
