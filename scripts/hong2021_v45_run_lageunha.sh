...
program=$repo/config/hong2021_v45_identifiable_mixture_copula_program.json
derived=$tng/derived/hong2021_v45
cache=$derived/conditioning_cache.h5
cache_report=$derived/conditioning_cache.json
checkpoint=$derived/identifiable_mixture_copula_step12000.pt
report=$derived/identifiable_mixture_copula_step12000.json
sequence=$tng/evaluation/tng100_simba_swift_v45_identifiable_mixture_sequence
preflight=$sequence/preflight.json
evaluation=$tng/evaluation/tng100_simba_swift_v45_e24_identifiable_mixture_copula
...
[[ $(hostname -s | tr '[:upper:]' '[:lower:]') == lageunha ]] || {
  echo "V45 requires Lageunha" >&2
  exit 1
}
[[ -z $(git status --porcelain) ]] || {
  echo "V45 requires clean worktree" >&2
  exit 1
}
for path in "$derived" "$sequence" "$evaluation"; do
  [[ ! -e $path ]] || {
    echo "V45 refuses existing output: $path" >&2
    exit 1
  }
done
...
printf "%s\n" training >"$status"
python -u src/hong2021_v45_train.py train \
  --program "$program" --repo "$repo" --cache "$cache" --cache-sha256 "$cache_sha" \
...
printf "%s\n" sampling >"$status"
python -u src/hong2021_v45_sample.py \
  --program "$program" --repo "$repo" --cache "$cache" --cache-sha256 "$cache_sha" \
...
printf "%s\n" evaluating >"$status"
for arm in identifiable_query_local_mixture_copula rolled_parameter_control structure_risk_ablation; do
...
printf "%s\n" gating >"$status"
python -u src/hong2021_v45_development_gate.py \
