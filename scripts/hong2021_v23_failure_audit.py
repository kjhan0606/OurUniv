#!/usr/bin/env python
"""Write the frozen automatic failure summary after a failed V23 gate."""
...
def main() -> None:
    ...
    if decision.get("development_pass") is not False:
        raise ValueError("V23 failure audit requires a failed decision")
    trajectory = {}
    for candidate in decision["candidates"]:
        trajectory[str(candidate["step"])] = {
            domain: {
                ...
                "Q6_std_maximum_deviation": row["conditional_Q6_std_no_harm"][
                    "maximum_absolute_generated_over_truth_std_minus_one"
                ],
                "Q6_std_threshold": row["conditional_Q6_std_no_harm"][
                    "threshold"
                ],
                "Q6_std_pass": row["conditional_Q6_std_no_harm"]["pass"],
            }
            for domain, row in candidate["domains"].items()
        }
    report = {
        "schema": "hong2021-v23-automatic-failure-audit-v1",
        ...
        "next": decision["next"],
        "coefficient_tuned_after_results": False,
        "horizon_extended_after_results": False,
        "Astrid_accessed": False,
        "historical_EAGLE_accessed": False,
    }
    if args.out.exists():
        raise RuntimeError(f"refusing to overwrite V23 failure audit: {args.out}")
