...
n=16; box=6.; positions=np.array([[(3+0.5)*box/n]*3,[(9+0.5)*box/n,(5+0.5)*box/n,(12+0.5)*box/n]])
vel=np.zeros((2,3)); masses=np.full((6,2),.7); exposure=np.full((6,n,n,n),.8)
...
for source,direction in enumerate(rh):
    kernel=cell_integrated_tsc_deposit(positions[source:source+1],np.array([1.]),np.array([direction]),np.array([disp]),n,box)
    # FFT convolution uses index 0 as the impulse origin; shift from the
    # actual cell-centre index of this impulse, not from n//2.
    center_index=np.floor(positions[source]/(box/n)).astype(int)
    kernels.append(np.fft.fftn(np.roll(kernel, -center_index, axis=(0,1,2))))
