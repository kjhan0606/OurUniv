...
training=$tng/training/tng100_simba_v11_recentered
evaluation=$tng/evaluation/tng100_simba_v11_recentered
...
tng_cache=$tng/derived/hong2021_v11/tng100_validation_corrected_fullband.h5
simba_dev_data=$simba/derived/hong2021_v1/simba_cv24_26_validation_all_observers.h5
simba_dev_cache=$simba/derived/hong2021_v11/simba_cv24_26_validation_corrected_fullband.h5
...
Path(p).write_text(json.dumps({
    ...
    "state":s,"detail":d,"host":socket.gethostname(),
...
if state!="complete": raise SystemExit(f"V11 training ended with {state}")
PY
...
for step in 002000 005000 010000; do
    ...
    python src/hong2021_residual_v11_recentered.py sample \
        ...
        --ensemble 16 --sampling-steps 40 --seed 19777 --device cuda
...
python src/hong2021_v9_development_gate.py \
    ...
    --report-schema hong2021-v11-full-fidelity-development-selection-v1 \
...
stress=$simba/evaluation/hong2021_v11_recentered_historical_stress
mkdir -p "$stress"
python src/hong2021_residual_v11_recentered.py sample \
    ...
    --ensemble 16 --sampling-steps 40 --seed 21777 --device cuda
python src/hong2021_residual_evaluate.py \
...
eagle_eval=$eagle/evaluation/hong2021_v11_recentered_confirmation32
mkdir -p "$eagle_eval"
...
python src/hong2021_residual_v11_recentered.py sample \
    ...
    --ensemble 16 --sampling-steps 40 --seed 22777 --device cuda
python src/hong2021_residual_evaluate.py \
...
python src/hong2021_hop_grid_gate.py \
    --edm "$selected_root/tng/ensemble16_steps40.h5" \
    --out "$selected_root/tng/grid_hop_gate.json" \
    --work "$selected_root/tng/grid_hop_work" --hop-dir "$hop_adapter" \
    --members 16 --objects 16 --workers 8 --voxel-mpc-h 0.3125 --omega-m 0.3
python src/hong2021_hop_grid_gate.py \
...
for label,value in zip(labels,sys.argv[2:],strict=True):
    path=Path(value); report=json.loads(path.read_text())["candidates"]["edm"]
    domains[label]={
        "decision":str(path.resolve()),
        "field_pass":report["field_gate"]["pass"],
        "grid_hop_pass":report["grid_hop_gate"]["pass"],
        "overall_pass":report["overall_pass"]
    }
accepted=all(row["overall_pass"] for row in domains.values())
result={
    "schema":"hong2021-v11-final-three-domain-gate-v1",
    "domains":domains,"accepted":accepted,
    "advance_to_forward_particle_dynamics":accepted
}
out.write_text(json.dumps(result,indent=2)+"\n")
print(json.dumps(result,indent=2))
PY
final_accept=$(python -c 'import json,sys;print("yes" if json.load(open(sys.argv[1]))["accepted"] else "no")' "$evaluation/final_gate_decision.json")
write_status complete "accepted=$final_accept; $evaluation/final_gate_decision.json"
