...
readonly runner_implementation_commit=f4e282cb1fe1e80a1184ead23d1fe5892a0c7c5e
...
readonly program="$repo/config/cf4_aggregate_evidence_smc_execution_authorization_program_v3.json"
readonly expected_program_sha=c899cff7b98038d5823d91623adc874c9699a0784fdb8c6718873a01e6883a2f
readonly grant="$repo/config/cf4_aggregate_evidence_smc_execution_grant_v3.json"
readonly release=/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_execution_authorization_v3_release.json
readonly implementation="$repo/src/cf4_aggregate_evidence_smc_execution_authorized_v3.py"
readonly runner="$repo/scripts/run_cf4_aggregate_evidence_smc_authorized_v3_lageunha.sh"
readonly status_checker="$repo/scripts/status_cf4_aggregate_evidence_smc_production.sh"
readonly data=/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v3
readonly state=/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_v3_run
...
from cf4_aggregate_evidence_smc_execution_authorized_v3 import (
...
set -o noclobber
{
    printf 'status=running\nstage=aggregate_evidence_smc_authorized_v3\n'
    printf 'pid=%s\nstarted_at=%s\nhost=%s\ngit_commit=%s\n' \
...
cd "$repo"
nice -n 5 "$python" src/cf4_aggregate_evidence_smc_execution_authorized_v3.py \
    --program "$program"
test -s "$result"
test -s "$manifest"
...
printf '[authorized-v3-runner] status=%s outcome=%s failure=%s\n' \
