...
readonly program="$repo/config/cf4_projection_nyquist_control_program.json"
...
exec 9>"$lock"
if ! flock -n 9; then
    echo "another Nyquist control owns $lock" >&2
    exit 75
fi
if [[ -e "$output" || -e "$running" || -e "$complete" || -e "$failed" \
      || -e "$log" || -e "$environment" ]]; then
    echo "refusing to overwrite a Nyquist-control lifecycle or output file" >&2
    exit 73
fi
if [[ ! -x "$python" || ! -f "$program" ]]; then
    echo "missing Python environment or frozen control program" >&2
    exit 66
fi
...
implementation_sha=$(sha256sum "$repo/src/cf4_projection_nyquist_control.py" | awk '{print $1}')
...
cd "$repo"
nice -n 5 "$python" src/cf4_projection_nyquist_control.py \
    --program "$program" \
    --out "$output"
test -s "$output"
