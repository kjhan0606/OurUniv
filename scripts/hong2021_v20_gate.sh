...
if [[ ${host,,} != lageunha ]]; then
  echo "V20-E8 gate must run on Lageunha, not $host" >&2
  exit 1
fi
...
from hong2021_v20_edm import load_frozen_registry
repo, registry, training, evaluation = map(Path, sys.argv[1:])
_, clean = git_state(repo)
if not clean:
    raise RuntimeError("V20-E8 gate requires a clean committed worktree")
load_frozen_registry(registry, repo)
run = json.loads((training / "run.json").read_text())
if run.get("status") != "complete":
    raise RuntimeError("V20-E8 training is not complete")
if evaluation.exists() and any(evaluation.iterdir()):
    raise RuntimeError(f"V20-E8 refuses pre-existing evaluation artifacts: {evaluation}")
PY

sample_evaluate() {
  local step=$1 domain=$2 root=$3
  if [[ -e $root ]]; then
    echo "V20-E8 refuses pre-existing candidate output: $root" >&2
    exit 1
  fi
  mkdir -p "$root"
  python src/hong2021_v20_edm.py sample \
...
python src/hong2021_v20_development_gate.py \
