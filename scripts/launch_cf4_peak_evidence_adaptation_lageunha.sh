...
readonly session=cf4-peak-adaptation-v1
readonly runner=/home/kjhan/BACKUP/CF4/scripts/run_cf4_peak_evidence_adaptation_lageunha.sh
...
if [[ -e "$state/result.json" || -e "$state/arrays.npz" \
    ...
    echo "adaptation output or lifecycle file already exists" >&2
    exit 73
fi
