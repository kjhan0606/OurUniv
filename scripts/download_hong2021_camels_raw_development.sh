...
python - "$suite" "$root" "$first" "$last" <<'PY'
...
suite, root_text, first_text, last_text = sys.argv[1:]
root = Path(root_text)
rows = []
for realization in range(int(first_text), int(last_text) + 1):
    ...
    with h5py.File(snapshot, "r") as handle:
        ...
        shape = tuple(handle["PartType1/Coordinates"].shape)
        if not np.isclose(float(header["BoxSize"]), 25000.0):
            raise SystemExit(f"unexpected snapshot BoxSize: {snapshot}")
        if abs(float(header["Redshift"])) > 1e-6:
            raise SystemExit(f"snapshot is not z=0: {snapshot}")
        if shape != (256**3, 3):
            raise SystemExit(f"unexpected DM coordinate shape {shape}: {snapshot}")
        snapshot_row = {
    ...
    with h5py.File(catalog, "r") as handle:
        ...
        if missing:
            raise SystemExit(f"catalog missing {missing}: {catalog}")
        if not np.isclose(float(header["BoxSize"]), 25000.0):
            raise SystemExit(f"unexpected catalog BoxSize: {catalog}")
        if abs(float(header["Redshift"])) > 1e-6:
            raise SystemExit(f"catalog is not z=0: {catalog}")
...
report = {
    "schema": "hong2021-v14-camels-raw-development-download-v1",
    "suite": suite,
    "role": "development_only",
    "realizations": list(range(int(first_text), int(last_text) + 1)),
    "validated_utc": datetime.now(timezone.utc).isoformat(),
    "rows": rows,
}
destination = root / "download" / "hong2021_v14_raw_development_manifest.json"
temporary = destination.with_suffix(".json.partial")
temporary.write_text(json.dumps(report, indent=2) + "\n")
os.replace(temporary, destination)
print(f"Validated {suite} raw V14 development data: {destination}")
PY
