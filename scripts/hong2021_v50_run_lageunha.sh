...
program=$repo/config/hong2021_v50_bounded_logit_mixture_copula_program.json
derived=$tng/derived/hong2021_v50
...
checkpoint=$derived/bounded_logit_mixture_copula_step12000.pt
report=$derived/bounded_logit_mixture_copula_step12000.json
sequence=$tng/evaluation/tng100_simba_swift_v50_bounded_logit_mixture_sequence
preflight=$sequence/preflight.json
evaluation=$tng/evaluation/tng100_simba_swift_v50_e26_bounded_logit_mixture_copula
...
[[ $(hostname -s | tr '[:upper:]' '[:lower:]') == lageunha ]] || {
  echo "V50 requires Lageunha" >&2
  exit 1
}
[[ -z $(git status --porcelain) ]] || {
  echo "V50 requires clean worktree" >&2
  exit 1
}
for path in "$derived" "$sequence" "$evaluation"; do
  [[ ! -e $path ]] || {
    echo "V50 refuses existing output: $path" >&2
    exit 1
  }
done
...
[[ $cache_sha == "$expected_cache_sha" ]] || {
  echo "V50 inherited cache hash differs" >&2
  exit 1
}

printf "%s\n" preflight >"$status"
python -u src/hong2021_v50_train.py preflight \
...
printf "%s\n" training >"$status"
python -u src/hong2021_v50_train.py train \
  --program "$program" --repo "$repo" --cache "$cache" --cache-sha256 "$cache_sha" \
...
printf "%s\n" sampling >"$status"
python -u src/hong2021_v50_sample.py \
  --program "$program" --repo "$repo" --cache "$cache" --cache-sha256 "$cache_sha" \
...
printf "%s\n" evaluating >"$status"
for arm in bounded_query_local_mixture_copula rolled_parameter_control structure_risk_ablation; do
...
printf "%s\n" gating >"$status"
python -u src/hong2021_v50_development_gate.py \
