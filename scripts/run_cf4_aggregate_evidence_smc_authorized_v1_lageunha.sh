...
readonly runner_implementation_commit=3db0e219a2569ed300e3647eca94a7078cae073c
...
readonly program="$repo/config/cf4_aggregate_evidence_smc_execution_authorization_program_v1.json"
readonly expected_program_sha=99539c20d873125aaece88239fb5615a1929db5f98e2fd84eb13f13da64bd841
readonly grant="$repo/config/cf4_aggregate_evidence_smc_execution_grant_v1.json"
readonly release=/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_execution_authorization_v1_release.json
readonly implementation="$repo/src/cf4_aggregate_evidence_smc_execution_authorized_v1.py"
readonly runner="$repo/scripts/run_cf4_aggregate_evidence_smc_authorized_v1_lageunha.sh"
...
from cf4_aggregate_evidence_smc_execution_authorized_v1 import (
...
set -o noclobber
{
    printf 'status=running\nstage=aggregate_evidence_smc_authorized_v1\n'
    printf 'pid=%s\nstarted_at=%s\nhost=%s\ngit_commit=%s\n' \
...
cd "$repo"
nice -n 5 "$python" src/cf4_aggregate_evidence_smc_execution_authorized_v1.py \
    --program "$program"
test -s "$result"
test -s "$manifest"
...
printf '[authorized-v1-runner] status=%s outcome=%s failure=%s\n' \
