...
readonly expected_program_sha=a4fcefa5d5bc163563df1bf5041c91415c3a4d6d8bdf20e710790e5e4e7f0b23
readonly grant="$repo/config/cf4_aggregate_evidence_smc_execution_grant_v4.json"
readonly release=/gpfs/kjhan/CF4/recon/linear_cr/aggregate_evidence_smc_execution_authorization_v4_release.json
readonly implementation="$repo/src/cf4_aggregate_evidence_smc_execution_authorized_v4.py"
...
set -o noclobber
{
    printf 'status=running\nstage=aggregate_evidence_smc_authorized_v4\n'
    printf 'pid=%s\nstarted_at=%s\nhost=%s\ngit_commit=%s\n' \
...
cd "$repo"
nice -n 5 "$python" src/cf4_aggregate_evidence_smc_execution_authorized_v4.py \
    --program "$program"
test -s "$result"
test -s "$manifest"
...
printf '[authorized-v4-runner] status=%s outcome=%s failure=%s\n' \
