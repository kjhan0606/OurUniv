...
[[ ${HOSTNAME,,} == lageunha ]] || { echo "V22 gate must run on Lageunha" >&2; exit 1; }
python - "$training" "$evaluation" <<'PY'
import json, sys
from pathlib import Path
training, evaluation = map(Path, sys.argv[1:])
if json.loads((training / "run.json").read_text()).get("status") != "complete":
    raise RuntimeError("V22 training is not complete")
if evaluation.exists() and any(evaluation.iterdir()):
    raise RuntimeError(f"V22 refuses pre-existing evaluation artifacts: {evaluation}")
PY

sample_evaluate() {
  local step=$1 domain=$2 root=$3
  mkdir -p "$root"
  python src/hong2021_v22_edm.py sample \
    --registry "$registry" --repo "$repo" --training-root "$training" \
    --domain "$domain" --step "$step" --out "$root/ensemble16_steps40.h5" --device cuda \
    >"$root/sample.log" 2>&1
...
python src/hong2021_v22_development_gate.py \
  --root "$evaluation/development_candidates" --training "$training" \
  --registry "$registry" --repo "$repo" --out "$evaluation/development_decision.json" \
