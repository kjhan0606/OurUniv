...
def fit_bias_radial_nb_survival(y, exposure, x, radius, train, wide=False):
    from scipy.optimize import minimize
    from scipy.special import gammaln
    ...
    def nll(t,yv,xv,rv,lev):
        ...
        return float(-np.sum(gammaln(yv+k)-gammaln(k)-gammaln(yv+1)+k*np.log(k/(k+mu))+yv*np.log(mu/(k+mu))))
    gamma_bounds=(.2,3.0) if wide else (.5,1.5)
    o=minimize(lambda t:nll(t,yy,xx,rt,le),[.8,.2,0,1],method='L-BFGS-B',bounds=[(.01,6),(-2,2),(-8,15),gamma_bounds])
