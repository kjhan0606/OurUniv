...
readonly session=cf4-peak-phase-v2
readonly runner=/home/kjhan/BACKUP/CF4/scripts/run_cf4_peak_evidence_phase_control_v2_lageunha.sh
...
if [[ -e "$state/result.json" || -e "$state/density_filter_rfft.npy" \
      || -e "$state/RUNNING" || -e "$state/COMPLETE" || -e "$state/FAILED" \
      || -e "$state/run.log" || -e "$state/environment.txt" ]]; then
    echo "v2 phase-control output or lifecycle file already exists" >&2; exit 73
fi
