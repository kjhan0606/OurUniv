...
cd "$repo"
exec 9>/gpfs/kjhan/.hong2021_locks/v23_e11_development.lock
flock -n 9 || { echo "another V23 supervisor holds the lock" >&2; exit 2; }
...
partial.write_text(json.dumps({
 "schema":"hong2021-v23-development-supervisor-status-v1", "state":state,
...
for _ in $(seq 1 30); do
  if pgrep -f "src/hong2021_v23_edm.py.*$training" >/dev/null || [[ -e $training/run.json ]]; then
    break
  fi
  sleep 2
done
...
while pgrep -f "src/hong2021_v23_edm.py.*$training" >/dev/null; do sleep 60; done
...
bash scripts/hong2021_v23_gate.sh >"$sequence/gate.log" 2>&1
...
if [[ $passed == True ]]; then
  ...
  python scripts/hong2021_v23_failure_audit.py --decision "$decision" \
