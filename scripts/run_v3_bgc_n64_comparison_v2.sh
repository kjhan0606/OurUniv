...
OUT="$ROOT/recon/linear_cr/v3_bgc_n64_comparison_v2"
mkdir -p "$OUT"
...
"$PYTHON" "$ROOT/src/cf4_bgc_cr_eval.py" \
  --config "$ROOT/config/v3_bgc_n64_comparison_v2.json" \
  --control "$OUT/manifest_v3_bgc_control_n64_paired.json" \
  --bgc "$OUT/manifest_v3_bgc_n64_paired.json" \
  --outdir "$OUT"
