...
sequence=$tng/evaluation/tng100_simba_swift_v21_sequence
status=$sequence/status.json
mkdir -p "$sequence" /gpfs/kjhan/.hong2021_locks
cd "$repo"

exec 9>/gpfs/kjhan/.hong2021_locks/v21_e9_development.lock
flock -n 9 || { echo "another V21 supervisor holds the lock" >&2; exit 2; }
...
partial.write_text(json.dumps({
    "schema": "hong2021-v21-development-supervisor-status-v1",
...
while pgrep -f "src/hong2021_v21_edm.py train.*$training" >/dev/null; do
  sleep 60
done
...
bash scripts/hong2021_v21_gate.sh >"$sequence/gate.log" 2>&1
...
if [[ $passed == True ]]; then
  ...
  write_status complete_failed_astrid_unopened "$next"
fi
