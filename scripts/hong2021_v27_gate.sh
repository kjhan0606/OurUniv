...
[[ ${HOSTNAME,,} == lageunha ]] || { echo "V27 gate requires Lageunha" >&2; exit 1; }
python - "$training" "$evaluation" <<'PY'
import json, sys
from pathlib import Path
training, evaluation = map(Path, sys.argv[1:])
if json.loads((training / "run.json").read_text()).get("status") != "complete":
    raise RuntimeError("V27 training is not complete")
if evaluation.exists() and any(evaluation.iterdir()):
    raise RuntimeError(f"V27 refuses pre-existing evaluation artifacts: {evaluation}")
PY

sample_evaluate() {
  local step=$1 domain=$2 root=$3
  mkdir -p "$root"
  python -u src/hong2021_v27.py sample \
...
python -u src/hong2021_v27_development_gate.py \
