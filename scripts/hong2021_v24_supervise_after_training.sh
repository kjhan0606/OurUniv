...
cd "$repo"
exec 9>/gpfs/kjhan/.hong2021_locks/v24_e12_development.lock
flock -n 9 || { echo "another V24 supervisor holds the lock" >&2; exit 2; }
...
partial.write_text(json.dumps({
 "schema":"hong2021-v24-development-supervisor-status-v1", "state":state,
...
for _ in $(seq 1 30); do
  if pgrep -f "src/hong2021_v24_edm.py.*$training" >/dev/null || [[ -e $training/run.json ]]; then
    break
  fi
  sleep 2
done
...
while pgrep -f "src/hong2021_v24_edm.py.*$training" >/dev/null; do sleep 60; done
...
bash scripts/hong2021_v24_gate.sh >"$sequence/gate.log" 2>&1
...
if [[ $passed == True ]]; then
  ...
  python scripts/hong2021_v24_failure_audit.py --decision "$decision" \
