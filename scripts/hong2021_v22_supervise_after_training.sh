...
training=$tng/training/tng100_simba_swift_v22_e10_long_horizon_edm
evaluation=$tng/evaluation/tng100_simba_swift_v22_e10_long_horizon
sequence=$tng/evaluation/tng100_simba_swift_v22_sequence
status=$sequence/status.json
mkdir -p "$sequence" /gpfs/kjhan/.hong2021_locks
cd "$repo"
exec 9>/gpfs/kjhan/.hong2021_locks/v22_e10_development.lock
flock -n 9 || { echo "another V22 supervisor holds the lock" >&2; exit 2; }
...
partial.write_text(json.dumps({
 "schema":"hong2021-v22-development-supervisor-status-v1", "state":state,
...
write_status monitoring_training "$training"
while pgrep -f "src/hong2021_v22_edm.py.*$training" >/dev/null; do sleep 60; done
state=$(python -c 'import json,sys; print(json.load(open(sys.argv[1])).get("status"))' "$training/run.json")
if [[ $state != complete ]]; then write_status training_failed "$state"; exit 1; fi
write_status running_development_sampling_gate "$evaluation"
bash scripts/hong2021_v22_gate.sh >"$sequence/gate.log" 2>&1
...
if [[ $passed == True ]]; then
  ...
  python scripts/hong2021_v22_failure_audit.py --decision "$decision" --out "$audit" >"$evaluation/automatic_failure_audit.log" 2>&1
