#!/usr/bin/env python
"""Write the automatic frozen summary after a failed V26 development gate."""
...
def main() -> None:
    ...
    if decision.get("development_pass") is not False:
        raise ValueError("V26 failure audit requires a failed decision")
    report = {
        "schema": "hong2021-v26-automatic-failure-audit-v1",
            ...
            str(candidate["step"]): {
                ...
                "domains": {
                    domain: {
                        ...
                        "comparison_to_v25": candidate["comparison_to_v25"][domain],
                    }
        ...
        "next": decision["next"],
        "spline_or_capacity_tuned_after_results": False,
    ...
    if args.out.exists():
        raise RuntimeError(f"refusing to overwrite V26 failure audit: {args.out}")
