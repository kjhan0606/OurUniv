...
program_sha=5a6c89fc032f33a1245f43d731189e5e4114c2e564acef55e96d2d62a45c03bb
freeze_commit=243fd09640782eaeef44b8dae85ab73fb58b1df4
...
sequence=$tng/evaluation/tng100_simba_swift_v84b_group_spliced_tail_sequence
preflight=$sequence/preflight.json
derived=$tng/derived/hong2021_v84b_group_spliced_tail
checkpoint=$derived/step12000.pt
training_report=$derived/training_report.json
group_gate=$tng/evaluation/tng100_simba_swift_v84b_group_spliced_tail_gate/decision.json
...
[[ $(sha256sum "$program" | awk '{print $1}') == "$program_sha" ]] || {
  echo "V84B frozen program differs" >&2
  exit 1
}
[[ $(sha256sum "$cache" | awk '{print $1}') == "$cache_sha" ]] || {
  echo "V84B conditioning cache differs" >&2
  exit 1
}
for path in "$sequence" "$derived" "$(dirname "$group_gate")"; do
  [[ ! -e $path ]] || {
    echo "V84B refuses existing output: $path" >&2
    exit 1
  }
done
...
record_failure() {
  ...
  if [[ $code -ne 0 ]]; then
    ...
    printf 'failed_terminal_V84B_no_retry exit=%s previous=%s\n' \
      "$code" "$previous" >"$status"
  fi
}
trap record_failure EXIT

printf '%s\n' testing_frozen_V84B >"$status"
...
printf '%s\n' training_fixed_12000_step_group_fit_V84B >"$status"
taskset -c 64-79 nice -n 10 \
...
printf '%s\n' evaluating_group_holdout_once_V84B >"$status"
taskset -c 64-79 nice -n 10 \
...
if [[ $(jq -r '.group_held_out_mechanism_pass' "$group_gate") == true ]]; then
  printf '%s\n' complete_V84B_group_gate_pass_waiting_result_record_before_any_production_refit >"$status"
else
  printf '%s\n' complete_V84B_group_gate_failure_all_other_payloads_locked >"$status"
fi
trap - EXIT
