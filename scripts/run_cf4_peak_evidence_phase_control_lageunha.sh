...
readonly program="$repo/config/cf4_peak_evidence_phase_control_program.json"
...
exec 9>"$lock"
if ! flock -n 9; then
    echo "another phase-cache control owns $lock" >&2
    exit 75
fi
if [[ -e "$output" || -e "$filter" || -e "$running" || -e "$complete" \
      || -e "$failed" || -e "$log" || -e "$environment" ]]; then
    echo "refusing to overwrite a phase-control lifecycle or output file" >&2
    exit 73
fi
if [[ ! -x "$python" || ! -f "$program" ]]; then
    echo "missing Python environment or frozen phase-control program" >&2
    exit 66
fi
