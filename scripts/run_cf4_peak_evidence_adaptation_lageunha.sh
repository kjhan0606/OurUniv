...
readonly runner="$repo/scripts/run_cf4_peak_evidence_adaptation_lageunha.sh"
readonly program="$repo/config/cf4_peak_evidence_adaptation_program.json"
readonly expected_program_sha=37942d212aec299296951cd66f6fa9040aa0ddd2987ee9c27cb25da5149ea9d6
readonly implementation="$repo/src/cf4_peak_evidence_adaptation.py"
readonly expected_implementation_sha=b5aa7b4f5c976532647ad0583e62e3395575cabed03a9e10199e5d7704f9424d
...
exec 9>"$lock"
if ! flock -n 9; then
    echo "another adaptation runner owns $lock" >&2
    exit 75
fi
...
if [[ -e "$output" || -e "$arrays" || -e "$proposal" || -e "$running" \
    ...
    echo "refusing to overwrite an adaptation file" >&2
    exit 73
fi
if [[ ! -x "$python" || ! -f "$program" || ! -f "$implementation" \
      || ! -f "$proposal_implementation" ]]; then
...
readonly program_sha implementation_sha proposal_implementation_sha runner_sha
if [[ "$program_sha" != "$expected_program_sha" \
    ...
    echo "program or implementation hash mismatch" >&2
    exit 65
fi

readonly -a science_paths=(
    config/cf4_peak_evidence_adaptation_program.json
    config/cf4_peak_evidence_adaptive_integration_design.json
    config/cf4_all_parent_peak_evidence_v1_result_record.json
    ...
    scripts/run_cf4_peak_evidence_adaptation_lageunha.sh
)
...
if ! git -C "$repo" diff --quiet HEAD -- "${science_paths[@]}"; then
    echo "adaptation science code/config differs from tracked HEAD" >&2
    exit 65
fi
...
finish() {
    ...
    if (( rc == 0 )) && [[ -s "$output" && -s "$arrays" \
        ...
        marker_tmp="$state/.COMPLETE.$$"
        {
            ...
            printf 'fallback_8192_adaptation_bank_authorized=%s\n' "$fallback_authorized"
...
running_tmp="$state/.RUNNING.$$"
{
    ...
    printf 'host=%s\ngit_commit=%s\n' "$host" "$commit"
    printf 'implementation_sha256=%s\nproposal_implementation_sha256=%s\nprogram_sha256=%s\n' \
        "$implementation_sha" "$proposal_implementation_sha" "$program_sha"
    printf 'runner_sha256=%s\n' "$runner_sha"
...
cd "$repo"
nice -n 5 "$python" src/cf4_peak_evidence_adaptation.py \
...
if result.get("schema") != "ouruniv-cf4-peak-evidence-adaptation-result-v1":
    raise SystemExit("unexpected adaptation result schema")
status = result.get("status")
allowed = {
    "complete_pass_freeze_defensive_final_proposal": True,
    "complete_fail_adaptation": False,
}
if status not in allowed:
    raise SystemExit("unexpected adaptation science status")
...
fallback = bool(decision.get("fallback_8192_adaptation_bank_authorized"))
if passed is not allowed[status] or proposal_frozen is not passed \
        or final_authorized is not passed:
    raise SystemExit("adaptation status and authorization disagree")
for key in ("conditional_field_bank_authorized", "candidate_generation_authorized",
    ...
    if decision.get(key) is not False:
        raise SystemExit("adaptation opened a forbidden downstream authorization")
lineage = result.get("lineage", {})
if pathlib.Path(lineage.get("arrays", "")) != arrays \
        or lineage.get("arrays_sha256") != sha256(arrays):
    raise SystemExit("adaptation array lineage mismatch")
if passed:
    if not proposal.is_file() or pathlib.Path(lineage.get("proposal", "")) != proposal \
            or lineage.get("proposal_sha256") != sha256(proposal):
        raise SystemExit("frozen proposal lineage mismatch")
elif proposal.exists():
    raise SystemExit("failed adaptation unexpectedly wrote a proposal")
for value in (status, passed, proposal_frozen, final_authorized, fallback):
    print(str(value).lower() if isinstance(value, bool) else value)
PY
)
if (( ${#validation[@]} != 5 )); then
    echo "adaptation result validation returned an incomplete record" >&2
    exit 65
fi
...
readonly fallback_authorized=${validation[4]}
printf '[runner] science_status=%s adaptation_pass=%s final_authorized=%s fallback_authorized=%s\n' \
    "$science_status" "$adaptation_pass" "$final_authorized" "$fallback_authorized"
