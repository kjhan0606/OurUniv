...
ssh -o BatchMode=yes "$node" \
    "cd $repo && exec setsid env CUDA_VISIBLE_DEVICES=$gpu_index SLURM_CPUS_PER_TASK=32 SLURM_JOB_ID=manual-$job_id SLURM_JOB_PARTITION=manual-tripwire OMP_NUM_THREADS=32 MKL_NUM_THREADS=32 OPENBLAS_NUM_THREADS=32 NUMEXPR_NUM_THREADS=32 bash scripts/run_lg_v8_manual_remote.sh $pidfile" \
    >"$stdout" 2>"$stderr" &
ssh_pid=$!
...
foreign_gpu_pids() {
    ...
    while IFS= read -r pid; do
        [[ -n "$pid" ]] || continue
        pgid=$(ssh -o BatchMode=yes "$node" \
            "ps -o pgid= -p $pid 2>/dev/null | tr -d '[:space:]'" || true)
        [[ "$pgid" == "$remote_pgid" ]] || printf '%s\n' "$pid"
...
while kill -0 "$ssh_pid" 2>/dev/null; do
    ...
    if scontrol show node "$node" | grep -Eq 'AllocTRES=.+(cpu|gres/gpu)'; then
        ...
        foreign_pids=$(foreign_gpu_pids)
        if [[ -n "$foreign_pids" ]]; then
            trip_reason="foreign_gpu_pids_${foreign_pids//$'\n'/_}"
        fi
    fi
